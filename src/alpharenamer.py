...
This is a module that performs alpha-renaming on a JavaScript AST generated by
pynarcissus to resolve scoping issues.

Workflow:
    AST -> alpharenamer.py -> alpha-renamed AST

Implementation Details:
1.  Receive AST. Let the variable ast hold the AST of some JavaScript code.

2.  Create frame representation of the AST by using the following function:

        frame_object = create_frame(ast)

    This function traverses the whole AST and creates a tree structure defined
    by the Frame class that represents the scope chain of the AST. The topmost
    Frame object defines the global scope, and is represented by the name
    __global__. This object then holds a reference to other functions, and thus
    the topmost frame holds the scope chain information for the whole JavaScript
    code.

    In addition, each frame object keeps track of all the identifiers used in
    its own scope.

3.  Perform alpha-renaming using the following function:
    
        frame_object = alpha_rename(frame_object, ast)

    This function performs alpha-renaming on the AST. Using the scope chain
    information in the frame object, this function resolves the variable names
    found in each function according to JavaScript scoping rules. Renamed
    variables are appended with a separator and a unique number. 

    After this function is executed, the properties of the nodes in the AST will
    change (mostly the name property, because that's what this module does,
    renaming).
"""
...
# Global Variables ---
VERBOSE = False
SEPARATOR = '_R_' #SEPARATOR is used to mark renamed variables
FUNCSEP   = '_R_' #FUNCSEP is used to mark renamed functions
...
def get_real_function_name(name):
    """Similar function to get_real_identifier, but for functions"""
    global FUNCSEP
    if FUNCSEP in name:
...
def isrenamed(name):
    """Returns True if a variable or a function has been renamed 
    
    name - a string containing a identifier
    """
...
class Call(object):
    ...
    def __init__(self, node):
        ...
        astutils.traverse_AST_level(node[0], append_name)
...
def create_frames(ast):
    ...
    astutils.traverse_AST_level(ast, create_frames_helper, signal_function_end)
    return ns.frame
...
def alpha_rename(frame, ast):
    ...
    def retrieve_unrenamed_identifiers(ast, frame):
        ...
        astutils.traverse_AST_level(ast, helper, signal_function_end)
