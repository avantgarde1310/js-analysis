...
% write_log('Caso 30 '),
     et_entries_by_goal(Tuple_Input, LstOutput), !.
...
%write_log('Caso 6 '),
        ...
        is_duplicate(Tuple_Input),
        ! ,
        et_entries_by_goal(Tuple_Input, LstOutput). %LstOutput unifies with the list of elements form the extension
...
cons_cl_Children(Subs,  [Pred|_Rp], [T_pred], []):-
       (Subs = [] -> ( build(Pred, New_input), et_entries_by_goal(New_input,LstOutput))
                   ;
...
et_entries_l([],[]).
et_entries_l([X|Xs],LstOutput):- et_entries_by_goal(X,L),
                                 et_entries_l(Xs,L2),
                                 append(L,L2,L3),
...
lineas(_Elemento,[],[]).
lineas(Elemento,[Car|Cdr],[L|Resto]) :- append(Elemento,Car, Laux),
                                        my_remove_duplicates(Laux,L),
                                        lineas(Elemento,Cdr,Resto).

my_filter([], []).
my_filter([X|R1], [X|R2]):- valid(X, true),!, my_filter(R1, R2).
my_filter([_X|R1], R2):- my_filter(R1, R2).
...
strategy([], _Vars, _N, _S, _L):- 
  write_log_list('Info: Nothing to debug.'), !.
...
write_answer( cl(Input,_Goal,Rule, _V, Ls, Fid, Output,_Ch,_State), Vars):-
     ...
     nl_log,
     write_log('With the rule: '),
     write_datalog_rule((Rule, Vars),0),
     nl_log,
     display_rule_info(Ls,Fid),
     %write_log(' Lines: '),
%     write_log(B),
