...
compute_test_case(ViewName,Class,Action) :-
  ...
  (var(TestCase),  
   !,
   write_log_list(['Info: The required test case cannot be generated.',nl])
   ;
	  tcs_to_facts(TestCase,Facts),
		recover_str_ctes_tuples(Facts,Dic,RTestCase),
    (Action==display,
     !,
     write_log_list(['Info: Test case over integers and strings:',nl]),
     write_quoted_log_list([RTestCase,nl])
     ;
     Action==add,
     !,
     rule_to_ruleNV_list(RTestCase,[],RNVs),
     assert_rules(RNVs,simplify,_,_)
     ;
     Action==replace,
     !,
     setof(TableName,TC^Args^(my_member(TC,RTestCase),TC=..[TableName|Args]),TableNames),
...
%  Index<0,
  ...
  CSs=[First|_],
  (First == '' ->
    write_log_list(['Error: Unable to generate a test case because there is no a string less than ''''.',nl]),
...
generate_test_case(MainV, Class, Dic, TestCase) :-    
	...
	(Error==true ->
	 ...
	 ;
    tc_size(N, _),
	  tc(N, MainV, Class, Dic, TestCase)
	).  

tc(N, MainV, all, Dic, TestCase) :- 
            fullTest(N, MainV, Dic, TestCase).             
            
tc(N, MainV, positive,  Dic, TestCase) :- 
            positiveTest(N, MainV,  Dic, TestCase). 
            
tc(N, MainV, negative,  Dic,TestCase) :- 
            negativeTest(N, MainV, Dic, TestCase).      
            
make_dic(ViewNames, Dic, Error) :-    
	my_nf_bagof(SCs,
	           ViewName^A^B^C^D^E^F^
	           (my_member(ViewName,ViewNames),
	...
	(L =< Max-Min+1 ->
	  ...
	  my_quicksort(UFSCss,Dic)
	 ;
	  write_log_list(['Error: Test case domain has not enough cardinality for, at least, ', L, ' string constants.',nl,
	                  '       Extend its bounds with the command /tc_domain <Min> <Max>',nl]),
	  Error=true).

%       
%  antiguo      
dependences(V, Tc_tables, Ordered_views):- 
...
orderViews(Arcs, [V|R1], [V|R2]) :- 
           isOrigen(Arcs, V, Exito), Exito == false, !, deleteArc(Arcs, V, Gd), 
           orderViews(Gd, R1, R2).             
orderViews(Arcs, [V|R1], L) :- 
           append(R1, [V], M),
           orderViews(Arcs, M, L).
           
isOrigen([], _V, false).    % la vista V no depende de otras vistas
isOrigen([F+_T|_Arcos], V, true) :-   % es origen
    F =..[_Barra, V|_Ar], !.                             
isOrigen([F-_T|_Arcos], V, true) :-   % es origen
    F =..[_Barra, V|_Ar], !.                             
isOrigen([_F+_T|Arcos],V,Val) :- 
    isOrigen(Arcos, V, Val).
isOrigen([_F-_T|Arcos],V,Val) :- 
    isOrigen(Arcos, V, Val).
...
fullTest(N, MainV, Dic, FLt) :- tc_size(_,Max),  M is Max + 1, M=N
                       ; 
                       (  M is N+1,
                          (   (setOfinstances(fullTC,MainV, Dic, FLt, ListT, _FLv, N), % pos y neg  
                                     
%                               labeling([ff, bisect, down], ListT),                                   
...
positiveTest(N, MainV, Dic, FLt) :- tc_size(_,Max),  M is Max + 1, M=N             
                         
                       ; 
                       (  M is N+1,
                          (   (setOfinstances(posTC, MainV, Dic, FLt, ListT, _FLv, N), %only pos
                                     
%                               labeling([ff, bisect, up], ListT), 
                               my_fd_labeling(ListT),                                   
                               !              
                               )
                           ; 
...
negativeTest(N, MainV, Dic, FLt) :- tc_size(_,Max),  M is Max + 1, M=N             
...
constraintsSelection(TC,FLt, Ftables, Prod, exists(C), B, InstView) :- 
          apply(TC, FLt, Ftables, sq(exists(C)), Prod, B, InstView).
                      
constraintsSelection(TC, FLt, Ftables, Prod, notExists(C), B, InstView) :- 
          apply(TC, FLt, Ftables, sq(notExists(C)), Prod, B, InstView).
...
%
apply(_, _FLt, _Ltables ,true, L, B, L) :- B#=1, !. %  At least, there exists a tuple in the product L

% At least, there exists a tuple matching the condition
apply(TC, _, _, bc(E1,Op,E2), Producto, B, InstView) :-  applyE(TC, _, _, bc(E1,Op,E2),  Producto, B, InstView). 
apply(TC, FLt, Ltab, and(C1, C2), Producto, B, InstView) :-  applyE(TC, FLt, Ltab, and(C1, C2),  Producto, B, InstView).
apply(TC, FLt, Ltab, or(C1, C2), Producto, B, InstView) :-  applyE(TC, FLt, Ltab, or(C1, C2),  Producto, B, InstView).
apply(TC, FLt, Ltab, sq(exists(C)), Prod, B, InstView) :- applyE(TC, FLt, Ltab, sq(exists(C)),  Prod, B, InstView).
apply(TC, FLt, Ltab, sq(notExists(C)), Prod, B, InstView) :- applyE(TC, FLt, Ltab, sq(notExists(C)),  Prod, B, InstView).
...
applyE(pos, FLt, Ltables, or(C1, C2), [Tuple|Lprod], B, [Tuple|Rt]) :- 
     applyTuple(pos,FLt, Ltables, or(C1, C2), Tuple, B1), B1#\=0,
...
applyE(pos, FLt, Ltables, and(C1, C2), [Tuple|Lprod], B, [Tuple|Rt]) :- 
     applyTuple(pos,FLt, Ltables, and(C1, C2), Tuple, B1), B1#\=0,
...
%-----     
applyE(_, _FLt, _Ltab, sq(exists(_C)), [], 0, []).
applyE(pos, FLt, Ltab, sq(exists(C)), [Tuple|Lprod], B, [Tuple|Rt]) :- 
     applyTuple(pos,FLt, Ltab, sq(exists(C)), Tuple, B1), B1#\=0,
     applyE(pos, FLt, Ltab, sq(exists(C)), Lprod, B2, Rt),  
     B #= B1+B2.
     
applyE(pos, FLt, Ltab, sq(exists(C)), [Tuple|Lprod], B, Rt) :- 
     applyTuple(pos,FLt, Ltab, sq(exists(C)), Tuple, B1), B1#=0,
     applyE(pos, FLt, Ltab, sq(exists(C)), Lprod, B2, Rt),  
     B #= B1+B2.
     
applyE(neg, FLt, Ltab, sq(exists(C)), [Tuple|Lprod], B, _) :- 
     applyTuple(neg,FLt, Ltab, sq(exists(C)), Tuple, B1), 
     applyE(neg, FLt, Ltab, sq(exists(C)), Lprod, B2, _),  
     B #= B1+B2.
%-----     
applyE(_, _FLt, _Ltab, sq(notExists(_C)), [], 0, []).
applyE(pos, FLt, Ltab, sq(notExists(C)), [Tuple|Lprod], B, [Tuple|Rt]) :- 
     applyTuple(pos,FLt, Ltab, sq(exists(C)), Tuple, B1), 
     B1#=0#<=>B3, 
     B3#\=0,
     applyE(pos, FLt, Ltab, sq(notExists(C)), Lprod, B2, Rt),  
     B #= B3+B2.   
     
applyE(pos, FLt, Ltab, sq(notExists(C)), [Tuple|Lprod], B, Rt) :- 
     applyTuple(pos,FLt, Ltab, sq(exists(C)), Tuple, B1), 
     B1#=0#<=>B3, 
     B3#=0,
     applyE(pos, FLt, Ltab, sq(notExists(C)), Lprod, B2, Rt),  
...
applyE(neg, FLt, Ltab, sq(notExists(C)), [Tuple|Lprod], B, _) :- 
     applyTuple(neg,FLt, Ltab, sq(exists(C)), Tuple, B1), 
     B1#=0#<=>B3,
     applyE(neg, FLt, Ltab, sq(notExists(C)), Lprod, B2, _),  
     B #= B3+B2. 
...
applyTuple(Tc,FLt, Ltables1, sq(exists(select(_,_,_La, 
                                        from(Ltables2), 
                                        where(Predicate),
                                        group_by(_), 
                                        having(_H),
...
applyTuple(Tc,FLt, Ltables1, sq(notExists(select(_,_,_La,
                                    from(Ltables2), 
                                    where(Predicate),
                                    group_by(_), 
                                    having(_H),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tuplasN(_Arity, 0, []).
tuplasN(Arity, N, [T|Resto]) :- N > 0, M is N-1,
                             tupla(Arity, T),
...
% get_foreign_key(Tabla , Lista de Foreign_key),       
  get_foreign_key(T,L) :- 
               setof((L1, RT, L2),my_foreign_key('$des',T ,L1, RT, L2),L), !.
