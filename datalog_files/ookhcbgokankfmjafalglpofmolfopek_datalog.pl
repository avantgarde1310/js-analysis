load('t0','chrome','i18n').
load('t1','t0','getMessage').
actual('0','1','noTitle').
callRet('0','noTitle').
assign('MSG_NO_TITLE_R_0','t2').
assign('POLL_INTERVAL_R_0','t3').
assign('DRAW_INTERVAL_R_0','t4').
assign('lastPollTime__R_0','0').
assign('badgeAnimation__R_0','__undefined__').
assign('canvasAnimation__R_0','__undefined__').
assign('nextEvent__R_0','null').
assign('eventList_R_0','eventList_R_0').
assign('nextEvents_R_0','nextEvents_R_0').
assign('calendars_R_0','calendars_R_0').
assign('pollUnderProgress_R_0','false').
assign('defaultAuthor_R_0','').
assign('isMultiCalendar_R_0','false').
assign('SINGLE_CALENDAR_SUPPORT_URL_R_0','t5').
assign('MULTIPLE_CALENDAR_SUPPORT_URL_R_0','t6').
assign('GOOGLE_CALENDAR_URL_R_0','http://www.google.com/calendar/render').
assign('DECLINED_URL_R_0','http://schemas.google.com/g/2005#event.declined').
assign('pendingLoadId__R_0','null').
ptsTo('BadgeAnimation_R_1','d_BadgeAnimation_R_1').
heapPtsTo('d_BadgeAnimation_R_1','prototype','p_BadgeAnimation_R_1').
prototype('p_BadgeAnimation_R_1','h_FP').
methodRet('d_BadgeAnimation_R_1','BadgeAnimation_R_1').
load('t7','BadgeAnimation_R_?','prototype').
load('t8','t7','paintFrame').
store('t7','paintFrame','lambda_R_2').
ptsTo('lambda_R_2','d_lambda_R_2').
heapPtsTo('d_lambda_R_2','prototype','p_lambda_R_2').
prototype('p_lambda_R_2','h_FP').
methodRet('d_lambda_R_2','lambda_R_2').
load('t9','BadgeAnimation_R_?','prototype').
load('t10','t9','start').
store('t9','start','lambda_R_3').
ptsTo('lambda_R_3','d_lambda_R_3').
heapPtsTo('d_lambda_R_3','prototype','p_lambda_R_3').
prototype('p_lambda_R_3','h_FP').
methodRet('d_lambda_R_3','lambda_R_3').
load('t11','BadgeAnimation_R_?','prototype').
load('t12','t11','stop').
store('t11','stop','lambda_R_5').
ptsTo('lambda_R_5','d_lambda_R_5').
heapPtsTo('d_lambda_R_5','prototype','p_lambda_R_5').
prototype('p_lambda_R_5','h_FP').
methodRet('d_lambda_R_5','lambda_R_5').
ptsTo('CanvasAnimation_R_6','d_CanvasAnimation_R_6').
heapPtsTo('d_CanvasAnimation_R_6','prototype','p_CanvasAnimation_R_6').
prototype('p_CanvasAnimation_R_6','h_FP').
methodRet('d_CanvasAnimation_R_6','CanvasAnimation_R_6').
load('t13','CanvasAnimation_R_?','prototype').
load('t14','t13','animate').
store('t13','animate','lambda_R_7').
ptsTo('lambda_R_7','d_lambda_R_7').
heapPtsTo('d_lambda_R_7','prototype','p_lambda_R_7').
prototype('p_lambda_R_7','h_FP').
methodRet('d_lambda_R_7','lambda_R_7').
load('t15','CanvasAnimation_R_?','prototype').
load('t16','t15','drawIconAtRotation').
store('t15','drawIconAtRotation','lambda_R_9').
ptsTo('lambda_R_9','d_lambda_R_9').
heapPtsTo('d_lambda_R_9','prototype','p_lambda_R_9').
prototype('p_lambda_R_9','h_FP').
methodRet('d_lambda_R_9','lambda_R_9').
load('t17','CanvasAnimation_R_?','prototype').
load('t18','t17','getSector').
store('t17','getSector','lambda_R_10').
ptsTo('lambda_R_10','d_lambda_R_10').
heapPtsTo('d_lambda_R_10','prototype','p_lambda_R_10').
prototype('p_lambda_R_10','h_FP').
methodRet('d_lambda_R_10','lambda_R_10').
formal('d_lambda_R_10','1','sector_R_10').
load('t19','CanvasAnimation_R_?','prototype').
load('t20','t19','drawFinal').
store('t19','drawFinal','lambda_R_11').
ptsTo('lambda_R_11','d_lambda_R_11').
heapPtsTo('d_lambda_R_11','prototype','p_lambda_R_11').
prototype('p_lambda_R_11','h_FP').
methodRet('d_lambda_R_11','lambda_R_11').
load('t21','CanvasAnimation_R_?','prototype').
load('t22','t21','showLoggedOut').
store('t21','showLoggedOut','lambda_R_13').
ptsTo('lambda_R_13','d_lambda_R_13').
heapPtsTo('d_lambda_R_13','prototype','p_lambda_R_13').
prototype('p_lambda_R_13','h_FP').
methodRet('d_lambda_R_13','lambda_R_13').
load('t23','CanvasAnimation_R_?','prototype').
load('t24','t23','getBadgeText').
store('t23','getBadgeText','lambda_R_14').
ptsTo('lambda_R_14','d_lambda_R_14').
heapPtsTo('d_lambda_R_14','prototype','p_lambda_R_14').
prototype('p_lambda_R_14','h_FP').
methodRet('d_lambda_R_14','lambda_R_14').
formal('d_lambda_R_14','1','nextEvent__R_14').
assign('CalendarManager_R_?','__objectInit__').
load('t25','CalendarManager_R_?','extractEvent').
store('CalendarManager_R_?','extractEvent','lambda_R_15').
ptsTo('lambda_R_15','d_lambda_R_15').
heapPtsTo('d_lambda_R_15','prototype','p_lambda_R_15').
prototype('p_lambda_R_15','h_FP').
methodRet('d_lambda_R_15','lambda_R_15').
formal('d_lambda_R_15','1','elem_R_15').
load('t26','CalendarManager_R_?','pollServer').
store('CalendarManager_R_?','pollServer','lambda_R_16').
ptsTo('lambda_R_16','d_lambda_R_16').
heapPtsTo('d_lambda_R_16','prototype','p_lambda_R_16').
prototype('p_lambda_R_16','h_FP').
methodRet('d_lambda_R_16','lambda_R_16').
load('t27','CalendarManager_R_?','genResponseChangeFunc').
store('CalendarManager_R_?','genResponseChangeFunc','lambda_R_18').
ptsTo('lambda_R_18','d_lambda_R_18').
heapPtsTo('d_lambda_R_18','prototype','p_lambda_R_18').
prototype('p_lambda_R_18','h_FP').
methodRet('d_lambda_R_18','lambda_R_18').
formal('d_lambda_R_18','1','xhr_R_18').
load('t28','CalendarManager_R_?','getCalendarFeed').
store('CalendarManager_R_?','getCalendarFeed','lambda_R_20').
ptsTo('lambda_R_20','d_lambda_R_20').
heapPtsTo('d_lambda_R_20','prototype','p_lambda_R_20').
prototype('p_lambda_R_20','h_FP').
methodRet('d_lambda_R_20','lambda_R_20').
formal('d_lambda_R_20','1','calendarId_R_20').
load('t29','CalendarManager_R_?','onCalendarResponse').
store('CalendarManager_R_?','onCalendarResponse','lambda_R_22').
ptsTo('lambda_R_22','d_lambda_R_22').
heapPtsTo('d_lambda_R_22','prototype','p_lambda_R_22').
prototype('p_lambda_R_22','h_FP').
methodRet('d_lambda_R_22','lambda_R_22').
formal('d_lambda_R_22','1','xmlhttp_R_22').
formal('d_lambda_R_22','2','calendarId_R_22').
load('t30','CalendarManager_R_?','parseCalendarEntry').
store('CalendarManager_R_?','parseCalendarEntry','lambda_R_24').
ptsTo('lambda_R_24','d_lambda_R_24').
heapPtsTo('d_lambda_R_24','prototype','p_lambda_R_24').
prototype('p_lambda_R_24','h_FP').
methodRet('d_lambda_R_24','lambda_R_24').
formal('d_lambda_R_24','1','responseXML_R_24').
formal('d_lambda_R_24','2','calendarId_R_24').
load('t31','CalendarManager_R_?','populateLatestEvent').
store('CalendarManager_R_?','populateLatestEvent','lambda_R_25').
ptsTo('lambda_R_25','d_lambda_R_25').
heapPtsTo('d_lambda_R_25','prototype','p_lambda_R_25').
prototype('p_lambda_R_25','h_FP').
methodRet('d_lambda_R_25','lambda_R_25').
formal('d_lambda_R_25','1','eventList_R_25').
assign('DATE_TIME_REGEX_R_0','{"regexp": "^(\\d\\d\\d\\d)-(\\d\\d)-(\\d\\d)T(\\d\\d):(\\d\\d):(\\d\\d)\\.\\d+(\\+|-)(\\d\\d):(\\d\\d)$", "modifiers": ""}').
assign('DATE_TIME_REGEX_Z_R_0','{"regexp": "^(\\d\\d\\d\\d)-(\\d\\d)-(\\d\\d)T(\\d\\d):(\\d\\d):(\\d\\d)\\.\\d+Z$", "modifiers": ""}').
assign('DATE_REGEX_R_0','{"regexp": "^(\\d\\d\\d\\d)-(\\d\\d)-(\\d\\d)$", "modifiers": ""}').
ptsTo('rfc3339StringToDate_R_26','d_rfc3339StringToDate_R_26').
heapPtsTo('d_rfc3339StringToDate_R_26','prototype','p_rfc3339StringToDate_R_26').
prototype('p_rfc3339StringToDate_R_26','h_FP').
methodRet('d_rfc3339StringToDate_R_26','rfc3339StringToDate_R_26').
formal('d_rfc3339StringToDate_R_26','1','rfc3339_R_26').
ptsTo('sortByDate_R_27','d_sortByDate_R_27').
heapPtsTo('d_sortByDate_R_27','prototype','p_sortByDate_R_27').
prototype('p_sortByDate_R_27','h_FP').
methodRet('d_sortByDate_R_27','sortByDate_R_27').
formal('d_sortByDate_R_27','1','event_1_R_27').
formal('d_sortByDate_R_27','2','event_2_R_27').
ptsTo('sortByAuthor_R_28','d_sortByAuthor_R_28').
heapPtsTo('d_sortByAuthor_R_28','prototype','p_sortByAuthor_R_28').
prototype('p_sortByAuthor_R_28','h_FP').
methodRet('d_sortByAuthor_R_28','sortByAuthor_R_28').
formal('d_sortByAuthor_R_28','1','event_1_R_28').
formal('d_sortByAuthor_R_28','2','event_2_R_28').
ptsTo('redraw_R_29','d_redraw_R_29').
heapPtsTo('d_redraw_R_29','prototype','p_redraw_R_29').
prototype('p_redraw_R_29','h_FP').
methodRet('d_redraw_R_29','redraw_R_29').
ptsTo('getCurrentTime_R_30','d_getCurrentTime_R_30').
heapPtsTo('d_getCurrentTime_R_30','prototype','p_getCurrentTime_R_30').
prototype('p_getCurrentTime_R_30','h_FP').
methodRet('d_getCurrentTime_R_30','getCurrentTime_R_30').
ptsTo('filterSpecialChar_R_31','d_filterSpecialChar_R_31').
heapPtsTo('d_filterSpecialChar_R_31','prototype','p_filterSpecialChar_R_31').
prototype('p_filterSpecialChar_R_31','h_FP').
methodRet('d_filterSpecialChar_R_31','filterSpecialChar_R_31').
formal('d_filterSpecialChar_R_31','1','data_R_31').
ptsTo('onSettingsChange_R_32','d_onSettingsChange_R_32').
heapPtsTo('d_onSettingsChange_R_32','prototype','p_onSettingsChange_R_32').
prototype('p_onSettingsChange_R_32','h_FP').
methodRet('d_onSettingsChange_R_32','onSettingsChange_R_32').
ptsTo('onTabUpdated_R_33','d_onTabUpdated_R_33').
heapPtsTo('d_onTabUpdated_R_33','prototype','p_onTabUpdated_R_33').
prototype('p_onTabUpdated_R_33','h_FP').
methodRet('d_onTabUpdated_R_33','onTabUpdated_R_33').
formal('d_onTabUpdated_R_33','1','tabId_R_33').
formal('d_onTabUpdated_R_33','2','changeInfo_R_33').
formal('d_onTabUpdated_R_33','3','tab_R_33').
ptsTo('onClickAction_R_34','d_onClickAction_R_34').
heapPtsTo('d_onClickAction_R_34','prototype','p_onClickAction_R_34').
prototype('p_onClickAction_R_34','h_FP').
methodRet('d_onClickAction_R_34','onClickAction_R_34').
ptsTo('isCalendarUrl_R_36','d_isCalendarUrl_R_36').
heapPtsTo('d_isCalendarUrl_R_36','prototype','p_isCalendarUrl_R_36').
prototype('p_isCalendarUrl_R_36','h_FP').
methodRet('d_isCalendarUrl_R_36','isCalendarUrl_R_36').
formal('d_isCalendarUrl_R_36','1','url_R_36').
ptsTo('init_R_37','d_init_R_37').
heapPtsTo('d_init_R_37','prototype','p_init_R_37').
prototype('p_init_R_37','h_FP').
methodRet('d_init_R_37','init_R_37').
load('t33','window','addEventListener_R_?').
actual('1','1','load').
callRet('1','load').
actual('1','2','init_R_?').
callRet('1','init_R_?').
actual('1','3','false').
callRet('1','false').
assign('t32','t34').
assign('isMultiCalendar_R_0','__undefined__').
load('t36','window','addEventListener').
actual('2','1','load').
callRet('2','load').
actual('2','2','init_R_?').
callRet('2','init_R_?').
actual('2','3','false').
callRet('2','false').
assign('t35','t37').
ptsTo('init_R_39','d_init_R_39').
heapPtsTo('d_init_R_39','prototype','p_init_R_39').
prototype('p_init_R_39','h_FP').
methodRet('d_init_R_39','init_R_39').
ptsTo('save_R_40','d_save_R_40').
heapPtsTo('d_save_R_40','prototype','p_save_R_40').
prototype('p_save_R_40','h_FP').
methodRet('d_save_R_40','save_R_40').
load('t39','chrome','extension').
load('t40','t39','onRequest').
load('t41','t40','addListener').
actual('3','1','lambda_R_41').
callRet('3','lambda_R_41').
assign('t38','t42').
ptsTo('lambda_R_41','d_lambda_R_41').
heapPtsTo('d_lambda_R_41','prototype','p_lambda_R_41').
prototype('p_lambda_R_41','h_FP').
methodRet('d_lambda_R_41','lambda_R_41').
formal('d_lambda_R_41','1','request_R_41').
formal('d_lambda_R_41','2','sender_R_41').
formal('d_lambda_R_41','3','sendResponse_R_41').
ptsTo('$_R_42','d_$_R_42').
heapPtsTo('d_$_R_42','prototype','p_$_R_42').
prototype('p_$_R_42','h_FP').
methodRet('d_$_R_42','$_R_42').
formal('d_$_R_42','1','id_R_42').
assign('t43','t44').
ptsTo('lambda_R_43','d_lambda_R_43').
heapPtsTo('d_lambda_R_43','prototype','p_lambda_R_43').
prototype('p_lambda_R_43','h_FP').
methodRet('d_lambda_R_43','lambda_R_43').
load('t45','this','timerId_').
store('this','timerId_','0').
load('t46','this','maxCount_').
store('this','maxCount_','8').
load('t47','this','current_').
store('this','current_','0').
load('t48','this','maxDot_').
store('this','maxDot_','4').
assign('text_R_2','').
load('t50','this','maxDot_').
assign('t49','t51').
assign('i_R_2','0').
assign('t53','i_R_2').
assign('t52','t53').
load('t54','this','current_').
assign('(None, "text_R_2")','.').
assign('text_R_2',' ').
assign('i_R_2','0').
load('t57','chrome','browserAction').
load('t58','t57','setBadgeText').
store('t59','text_R_2','text_R_2').
actual('5','1','t59').
callRet('5','t59').
assign('t56','t60').
load('t62','this','current_').
assign('t63','t62').
assign('t61','t63').
load('t65','this','current_').
load('t66','this','maxCount_').
assign('t64','t67').
load('t68','this','current_').
store('this','current_','0').
load('t70','this','timerId_').
assign('t69','t70').
methodRet('d_lambda_R_3','null').
assign('self_R_3','this').
load('t71','this','timerId_').
load('t72','window','setInterval_R_?').
actual('6','1','lambda_R_4').
callRet('6','lambda_R_4').
actual('6','2','100').
callRet('6','100').
store('this','timerId_','t73').
ptsTo('lambda_R_4','d_lambda_R_4').
heapPtsTo('d_lambda_R_4','prototype','p_lambda_R_4').
prototype('p_lambda_R_4','h_FP').
methodRet('d_lambda_R_4','lambda_R_4').
load('t75','self_R_3','paintFrame_R_?').
assign('t74','t76').
load('t78','this','timerId_').
assign('t79','t78').
assign('t77','t79').
methodRet('d_lambda_R_5','null').
load('t81','window','clearInterval_R_?').
load('t82','this','timerId_').
actual('8','1','t82').
callRet('8','t82').
assign('t80','t83').
load('t84','this','timerId_').
store('this','timerId_','0').
load('t85','this','animationFrames_').
store('this','animationFrames_','36').
load('t86','this','animationSpeed_').
store('this','animationSpeed_','10').
load('t87','this','canvas_').
actual('9','1','canvas').
callRet('9','canvas').
store('this','canvas_','t88').
load('t89','this','canvasContext_').
load('t90','this','canvas_').
load('t91','t90','getContext_R_?').
actual('10','1','"2d"').
callRet('10','"2d"').
store('this','canvasContext_','t92').
load('t93','this','loggedInImage_').
actual('11','1','logged_in').
callRet('11','logged_in').
store('this','loggedInImage_','t94').
load('t95','this','rotation_').
store('this','rotation_','0').
load('t96','this','w').
load('t97','this','canvas_').
load('t98','t97','width').
store('this','w','t98').
load('t99','this','h').
load('t100','this','canvas_').
load('t101','t100','height').
store('this','h','t101').
load('t102','this','RED').
store('t102','0','208').
store('t102','1','0').
store('t102','2','24').
store('t102','3','255').
store('this','RED','t102').
load('t103','this','BLUE').
store('t103','0','0').
store('t103','1','24').
store('t103','2','208').
store('t103','3','255').
store('this','BLUE','t103').
load('t104','this','currentBadge_').
store('this','currentBadge_','null').
load('t105','this','rotation_').
load('t106','this','animationFrames_').
store('this','rotation_','t107').
load('t109','this','drawIconAtRotation_R_?').
assign('t108','t110').
assign('self_R_7','this').
load('t112','this','rotation_').
assign('t111','t113').
load('t115','this','drawFinal_R_?').
assign('t114','t116').
load('t118','self_R_7','animationSpeed_').
actual('14','1','lambda_R_8').
callRet('14','lambda_R_8').
actual('14','2','t118').
callRet('14','t118').
assign('t117','t119').
ptsTo('lambda_R_8','d_lambda_R_8').
heapPtsTo('d_lambda_R_8','prototype','p_lambda_R_8').
prototype('p_lambda_R_8','h_FP').
methodRet('d_lambda_R_8','lambda_R_8').
load('t121','self_R_7','animate_R_?').
assign('t120','t122').
load('t124','this','canvasContext_').
load('t125','t124','save_R_?').
assign('t123','t126').
load('t128','this','canvasContext_').
load('t129','t128','clearRect_R_?').
load('t130','this','w').
load('t131','this','h').
actual('17','1','0').
callRet('17','0').
actual('17','2','0').
callRet('17','0').
actual('17','3','t130').
callRet('17','t130').
actual('17','4','t131').
callRet('17','t131').
assign('t127','t132').
load('t134','this','canvasContext_').
load('t135','t134','translate_R_?').
load('t136','Math_R_?','ceil_R_?').
load('t137','this','w').
actual('18','1','t138').
callRet('18','t138').
load('t140','Math_R_?','ceil').
load('t141','this','h').
actual('19','1','t142').
callRet('19','t142').
actual('20','1','t139').
callRet('20','t139').
actual('20','2','t143').
callRet('20','t143').
assign('t133','t144').
load('t146','this','canvasContext_').
load('t147','t146','rotate_R_?').
load('t148','Math_R_?','PI').
load('t150','this','getSector_R_?').
load('t151','this','rotation_').
actual('21','1','t151').
callRet('21','t151').
actual('22','1','t153').
callRet('22','t153').
assign('t145','t154').
load('t156','this','canvasContext_').
load('t157','t156','drawImage_R_?').
load('t158','this','loggedInImage_').
load('t159','Math_R_?','ceil').
load('t160','this','w').
actual('23','1','t161').
callRet('23','t161').
assign('t163','t162').
load('t164','Math_R_?','ceil').
load('t165','this','h').
actual('24','1','t166').
callRet('24','t166').
assign('t168','t167').
actual('25','1','t158').
callRet('25','t158').
actual('25','2','t163').
callRet('25','t163').
actual('25','3','t168').
callRet('25','t168').
assign('t155','t169').
load('t171','this','canvasContext_').
load('t172','t171','restore_R_?').
assign('t170','t173').
load('t175','chrome','browserAction').
load('t176','t175','setIcon').
load('t178','this','canvasContext_').
load('t179','t178','getImageData_R_?').
load('t180','this','w').
load('t181','this','h').
actual('27','1','0').
callRet('27','0').
actual('27','2','0').
callRet('27','0').
actual('27','3','t180').
callRet('27','t180').
actual('27','4','t181').
callRet('27','t181').
store('t177','imageData_R_?','t182').
actual('28','1','t177').
callRet('28','t177').
assign('t174','t183').
load('t184','Math_R_?','sin_R_?').
load('t185','Math_R_?','PI').
load('t187','Math_R_?','PI').
actual('29','1','t189').
callRet('29','t189').
methodRet('d_lambda_R_10','t192').
load('t194','badgeAnimation__R_0','stop_R_?').
assign('t193','t195').
assign('t197','nextEvent__R_0').
assign('t196','t197').
load('t199','this','drawIconAtRotation_R_?').
assign('t198','t200').
load('t201','this','rotation_').
store('this','rotation_','0').
load('t202','nextEvent__R_0','startTime').
load('t203','t202','getTime_R_?').
assign('ms_R_11','t206').
assign('nextEventMin_R_11','t208').
load('t210','this','RED').
load('t211','this','BLUE').
assign('(None, "bgColor_R_11")','t210').
assign('bgColor_R_11','t211').
load('t213','chrome','browserAction').
load('t214','t213','setBadgeBackgroundColor').
store('t215','color_R_?','bgColor_R_11').
actual('34','1','t215').
callRet('34','t215').
assign('t212','t216').
load('t217','this','getBadgeText_R_?').
actual('35','1','nextEvent__R_0').
callRet('35','nextEvent__R_0').
assign('currentBadge__R_?','t218').
load('t220','chrome','browserAction').
load('t221','t220','setBadgeText').
store('t222','text_R_11','currentBadge__R_?').
actual('36','1','t222').
callRet('36','t222').
assign('t219','t223').
load('t225','nextEvents_R_0','length').
assign('t224','t226').
assign('text_R_11','').
load('t227','nextEvents_R_0','i_R_11').
assign('event_R_?','t227').
assign('i_R_11','0').
assign('event_R_?','__undefined__').
assign('t229','i_R_11').
assign('t228','t229').
load('t230','event_R_?','title').
assign('text_R_11','t230').
load('t232','event_R_?','author').
load('t233','event_R_?','location').
assign('t231','t234').
assign('text_R_11','
').
load('t236','event_R_?','location').
assign('t235','t236').
load('t237','event_R_?','location').
assign('text_R_11','t238').
load('t240','event_R_?','author').
assign('t239','t240').
load('t241','event_R_?','author').
assign('text_R_11','t241').
load('t243','nextEvents_R_0','length').
assign('t242','t245').
assign('text_R_11','
----------
').
assign('i_R_11','0').
assign('event_R_?','__undefined__').
actual('37','1','text_R_11').
callRet('37','text_R_11').
assign('text_R_11','t246').
load('t248','chrome','browserAction').
load('t249','t248','setTitle').
store('t250','title','text_R_11').
actual('38','1','t250').
callRet('38','t250').
assign('t247','t251').
load('t253','this','showLoggedOut_R_?').
assign('t252','t254').
assign('pollUnderProgress_R_0','false').
load('t256','chrome','extension').
load('t257','t256','sendRequest').
store('t258','message_R_?','enableSave').
actual('40','1','t258').
callRet('40','t258').
actual('40','2','lambda_R_12').
callRet('40','lambda_R_12').
assign('t255','t259').
ptsTo('lambda_R_12','d_lambda_R_12').
heapPtsTo('d_lambda_R_12','prototype','p_lambda_R_12').
prototype('p_lambda_R_12','h_FP').
methodRet('d_lambda_R_12','lambda_R_12').
methodRet('d_lambda_R_11','null').
assign('currentBadge__R_?','?').
load('t261','chrome','browserAction').
load('t262','t261','setIcon').
store('t263','path_R_?','../images/icon-16_bw.gif').
actual('41','1','t263').
callRet('41','t263').
assign('t260','t264').
load('t266','chrome','browserAction').
load('t267','t266','setBadgeBackgroundColor').
store('t269','0','190').
store('t269','1','190').
store('t269','2','190').
store('t269','3','230').
store('t268','color_R_?','t269').
actual('42','1','t268').
callRet('42','t268').
assign('t265','t270').
load('t272','chrome','browserAction').
load('t273','t272','setBadgeText').
store('t274','text_R_?','?').
actual('43','1','t274').
callRet('43','t274').
assign('t271','t275').
load('t277','chrome','browserAction').
load('t278','t277','setTitle').
store('t279','title','').
actual('44','1','t279').
callRet('44','t279').
assign('t276','t280').
assign('t282','nextEvent__R_14').
assign('t281','t282').
methodRet('d_lambda_R_14','').
load('t283','nextEvent__R_14','startTime').
load('t284','t283','getTime_R_?').
assign('ms_R_14','t287').
load('t288','Math_R_?','ceil_R_?').
actual('47','1','t290').
callRet('47','t290').
assign('nextEventMin_R_14','t291').
assign('text_R_14','').
assign('t292','t293').
assign('t294','t295').
assign('t296','t298').
load('t299','chrome','i18n').
load('t300','t299','getMessage').
load('t301','Math_R_?','round_R_?').
actual('48','1','t303').
callRet('48','t303').
load('t305','t304','toString_R_?').
actual('50','1','days').
callRet('50','days').
actual('50','2','t306').
callRet('50','t306').
assign('text_R_14','t307').
load('t308','chrome','i18n').
load('t309','t308','getMessage').
load('t310','Math_R_?','round').
actual('51','1','t311').
callRet('51','t311').
load('t313','t312','toString').
actual('53','1','hours').
callRet('53','hours').
actual('53','2','t314').
callRet('53','t314').
assign('text_R_14','t315').
load('t316','chrome','i18n').
load('t317','t316','getMessage').
load('t318','nextEventMin_R_14','toString').
actual('55','1','minutes').
callRet('55','minutes').
actual('55','2','t319').
callRet('55','t319').
assign('text_R_14','t320').
methodRet('d_lambda_R_14','text_R_14').
assign('out_R_15','__objectInit__').
assign('t321','t322').
load('t323','elem_R_15','firstChild').
assign('node_R_15','t323').
load('t324','node_R_15','nextSibling').
assign('node_R_15','t324').
load('t326','node_R_15','nodeName').
assign('t325','t327').
load('t329','node_R_15','nodeName').
load('t331','node_R_15','getAttribute').
actual('56','1','rel').
callRet('56','rel').
assign('t328','t334').
load('t336','node_R_15','nodeName').
assign('t335','t337').
load('t339','node_R_15','nodeName').
assign('t338','t340').
load('t342','node_R_15','nodeName').
assign('t341','t343').
load('t345','node_R_15','nodeName').
assign('t344','t346').
load('t347','node_R_15','getAttribute_R_?').
actual('57','1','startTime').
callRet('57','startTime').
assign('startTimeStr_R_15','t348').
load('t349','node_R_15','getAttribute').
actual('58','1','endTime').
callRet('58','endTime').
assign('endTimeStr_R_15','t350').
actual('59','1','startTimeStr_R_15').
callRet('59','startTimeStr_R_15').
assign('startTime_R_?','t351').
actual('60','1','endTimeStr_R_15').
callRet('60','endTimeStr_R_15').
assign('endTime_R_?','t352').
assign('t353','t356').
load('t357','out_R_15','isAllDay').
load('t358','startTimeStr_R_15','length').
store('out_R_15','isAllDay','t359').
load('t360','out_R_15','startTime').
store('out_R_15','startTime','startTime_R_?').
load('t361','out_R_15','endTime').
store('out_R_15','endTime','endTime_R_?').
load('t362','out_R_15','status').
load('t363','node_R_15','getAttribute').
actual('61','1','value').
callRet('61','value').
store('out_R_15','status','t364').
load('t366','node_R_15','firstChild').
assign('t365','t366').
load('t367','out_R_15','attendeeStatus').
load('t368','node_R_15','firstChild').
load('t369','t368','getAttribute').
actual('62','1','value').
callRet('62','value').
store('out_R_15','attendeeStatus','t370').
load('t371','out_R_15','location').
load('t372','node_R_15','getAttribute').
actual('63','1','valueString').
callRet('63','valueString').
store('out_R_15','location','t373').
load('t374','out_R_15','url').
load('t375','node_R_15','getAttribute').
actual('64','1','href').
callRet('64','href').
store('out_R_15','url','t376').
load('t377','out_R_15','title').
load('t378','node_R_15','firstChild').
load('t379','node_R_15','firstChild').
load('t380','t379','nodeValue').
assign('(deque(["out_R_15", "title"]), "t377")','t380').
store('out_R_15','title','MSG_NO_TITLE_R_0').
load('t381','elem_R_15','firstChild').
assign('node_R_15','t381').
methodRet('d_lambda_R_15','out_R_15').
assign('t383','pollUnderProgress_R_0').
assign('t382','t383').
assign('eventList_R_0','eventList_R_0').
assign('pollUnderProgress_R_0','true').
assign('pendingLoadId__R_0','null').
assign('calendars_R_0','calendars_R_0').
assign('lastPollTime__R_0','t384').
assign('url_R_16','__undefined__').
ptsTo('t385','n_XMLHttpRequest_R_?_0').
prototype('n_XMLHttpRequest_R_?_0', 'p_XMLHttpRequest_R_?').
assign('xhr_R_16','t385').
load('t386','xhr_R_16','onreadystatechange').
load('t387','CalendarManager_R_?','genResponseChangeFunc_R_?').
actual('66','1','xhr_R_16').
callRet('66','xhr_R_16').
store('xhr_R_16','onreadystatechange','t388').
load('t389','xhr_R_16','onerror').
store('xhr_R_16','onerror','lambda_R_17').
ptsTo('lambda_R_17','d_lambda_R_17').
heapPtsTo('d_lambda_R_17','prototype','p_lambda_R_17').
prototype('p_lambda_R_17','h_FP').
methodRet('d_lambda_R_17','lambda_R_17').
formal('d_lambda_R_17','1','error_R_17').
assign('t390','isMultiCalendar_R_0').
assign('url_R_16','SINGLE_CALENDAR_SUPPORT_URL_R_0').
assign('url_R_16','MULTIPLE_CALENDAR_SUPPORT_URL_R_0').
load('t392','xhr_R_16','open_R_?').
actual('67','1','GET').
callRet('67','GET').
actual('67','2','url_R_16').
callRet('67','url_R_16').
assign('t391','t393').
load('t395','xhr_R_16','send_R_?').
actual('68','1','null').
callRet('68','null').
assign('t394','t396').
load('t398','console','log_R_?').
actual('69','1','t399').
callRet('69','t399').
assign('t397','t400').
assign('nextEvent__R_0','null').
load('t402','canvasAnimation__R_0','drawFinal_R_?').
assign('t401','t403').
methodRet('d_lambda_R_18','lambda_R_19').
ptsTo('lambda_R_19','d_lambda_R_19').
heapPtsTo('d_lambda_R_19','prototype','p_lambda_R_19').
prototype('p_lambda_R_19','h_FP').
methodRet('d_lambda_R_19','lambda_R_19').
load('t405','xhr_R_18','readyState').
assign('t404','t406').
methodRet('d_lambda_R_19','null').
load('t408','xhr_R_18','responseXML').
assign('t409','t408').
assign('t407','t409').
load('t411','console','log_R_?').
actual('71','1','No responseXML').
callRet('71','No responseXML').
assign('t410','t412').
assign('nextEvent__R_0','null').
load('t414','canvasAnimation__R_0','drawFinal_R_?').
assign('t413','t415').
methodRet('d_lambda_R_19','null').
assign('t416','isMultiCalendar_R_0').
assign('calendars_R_0','calendars_R_0').
load('t418','calendars_R_0','push_R_?').
actual('73','1','SINGLE_CALENDAR_SUPPORT_URL_R_0').
callRet('73','SINGLE_CALENDAR_SUPPORT_URL_R_0').
assign('t417','t419').
load('t421','CalendarManager_R_?','parseCalendarEntry_R_?').
load('t422','xhr_R_18','responseXML').
actual('74','1','t422').
callRet('74','t422').
actual('74','2','0').
callRet('74','0').
assign('t420','t423').
methodRet('d_lambda_R_19','null').
load('t424','xhr_R_18','responseXML').
load('t425','t424','getElementsByTagName_R_?').
actual('75','1','entry').
callRet('75','entry').
assign('entry__R_19','t426').
load('t428','entry__R_19','length').
assign('t427','t430').
assign('calendars_R_0','calendars_R_0').
load('t431','entry__R_19','i_R_19').
assign('entry_R_?','t431').
assign('i_R_19','0').
assign('entry_R_?','__undefined__').
assign('t433','i_R_19').
assign('t432','t433').
assign('t435','i_R_19').
assign('t434','t435').
load('t436','entry_R_?','querySelector_R_?').
actual('76','1','title').
callRet('76','title').
load('t438','t437','textContent').
assign('defaultAuthor_R_0','t438').
load('t439','entry_R_?','querySelector').
actual('77','1','hidden').
callRet('77','hidden').
assign('isHidden_R_19','t440').
load('t441','entry_R_?','querySelector').
actual('78','1','selected').
callRet('78','selected').
assign('isSelected_R_19','t442').
load('t444','isHidden_R_19','getAttribute').
actual('79','1','value').
callRet('79','value').
assign('t443','t447').
load('t449','isSelected_R_19','getAttribute').
actual('80','1','value').
callRet('80','value').
assign('t448','t452').
load('t453','entry_R_?','querySelector').
actual('81','1','content').
callRet('81','content').
assign('calendar_content_R_19','t454').
load('t455','calendar_content_R_19','getAttribute_R_?').
actual('82','1','src').
callRet('82','src').
assign('cal_src_R_19','t456').
assign('cal_src_R_19','?toolbar=true&max-results=10').
load('t458','calendars_R_0','push').
actual('83','1','cal_src_R_19').
callRet('83','cal_src_R_19').
assign('t457','t459').
assign('i_R_19','0').
assign('entry_R_?','__undefined__').
load('t461','CalendarManager_R_?','getCalendarFeed_R_?').
actual('84','1','0').
callRet('84','0').
assign('t460','t462').
methodRet('d_lambda_R_19','null').
load('t464','console','error_R_?').
actual('85','1','Error: feed retrieved, but no event found').
callRet('85','Error: feed retrieved, but no event found').
assign('t463','t465').
assign('nextEvent__R_0','null').
load('t467','canvasAnimation__R_0','drawFinal').
assign('t466','t468').
ptsTo('t469','n_XMLHttpRequest_R_?_1').
prototype('n_XMLHttpRequest_R_?_1', 'p_XMLHttpRequest_R_?').
assign('xmlhttp_R_20','t469').
load('t470','xmlhttp_R_20','onreadystatechange').
load('t471','CalendarManager_R_?','onCalendarResponse_R_?').
actual('87','1','xmlhttp_R_20').
callRet('87','xmlhttp_R_20').
actual('87','2','calendarId_R_20').
callRet('87','calendarId_R_20').
store('xmlhttp_R_20','onreadystatechange','t472').
load('t473','xmlhttp_R_20','onerror').
store('xmlhttp_R_20','onerror','lambda_R_21').
ptsTo('lambda_R_21','d_lambda_R_21').
heapPtsTo('d_lambda_R_21','prototype','p_lambda_R_21').
prototype('p_lambda_R_21','h_FP').
methodRet('d_lambda_R_21','lambda_R_21').
formal('d_lambda_R_21','1','error_R_21').
load('t475','xmlhttp_R_20','open_R_?').
load('t476','calendars_R_0','calendarId_R_20').
actual('88','1','GET').
callRet('88','GET').
actual('88','2','t476').
callRet('88','t476').
assign('t474','t477').
load('t479','xmlhttp_R_20','send_R_?').
actual('89','1','null').
callRet('89','null').
assign('t478','t480').
load('t482','console','log_R_?').
actual('90','1','t483').
callRet('90','t483').
assign('t481','t484').
assign('nextEvent__R_0','null').
load('t486','canvasAnimation__R_0','drawFinal_R_?').
assign('t485','t487').
methodRet('d_lambda_R_22','lambda_R_23').
ptsTo('lambda_R_23','d_lambda_R_23').
heapPtsTo('d_lambda_R_23','prototype','p_lambda_R_23').
prototype('p_lambda_R_23','h_FP').
methodRet('d_lambda_R_23','lambda_R_23').
load('t489','xmlhttp_R_22','readyState').
assign('t488','t490').
methodRet('d_lambda_R_23','null').
load('t492','xmlhttp_R_22','responseXML').
assign('t493','t492').
assign('t491','t493').
load('t495','console','log_R_?').
actual('92','1','No responseXML').
callRet('92','No responseXML').
assign('t494','t496').
assign('nextEvent__R_0','null').
load('t498','canvasAnimation__R_0','drawFinal_R_?').
assign('t497','t499').
methodRet('d_lambda_R_23','null').
load('t501','CalendarManager_R_?','parseCalendarEntry_R_?').
load('t502','xmlhttp_R_22','responseXML').
actual('94','1','t502').
callRet('94','t502').
actual('94','2','calendarId_R_22').
callRet('94','calendarId_R_22').
assign('t500','t503').
load('t504','responseXML_R_24','getElementsByTagName_R_?').
actual('95','1','entry').
callRet('95','entry').
assign('entry__R_24','t505').
load('t506','responseXML_R_24','querySelector_R_?').
actual('96','1','author name').
callRet('96','author name').
load('t508','t507','textContent').
assign('author_R_24','t508').
load('t510','entry__R_24','length').
assign('t509','t512').
load('t513','entry__R_24','i_R_24').
assign('entry_R_?','t513').
assign('i_R_24','0').
assign('entry_R_?','__undefined__').
assign('t515','i_R_24').
assign('t514','t515').
load('t516','CalendarManager_R_?','extractEvent_R_?').
actual('97','1','entry_R_?').
callRet('97','entry_R_?').
assign('event__R_24','t517').
load('t519','event__R_24','startTime').
assign('t518','t519').
load('t520','event__R_24','startTime').
load('t521','t520','getTime_R_?').
assign('t_R_24','t524').
load('t527','event__R_24','attendeeStatus').
assign('t525','t529').
assign('t530','isMultiCalendar_R_0').
load('t531','event__R_24','author').
store('event__R_24','author','author_R_24').
load('t533','eventList_R_0','push_R_?').
actual('100','1','event__R_24').
callRet('100','event__R_24').
assign('t532','t534').
assign('i_R_24','0').
assign('entry_R_?','__undefined__').
assign('t536','calendarId_R_24').
assign('t535','t536').
load('t538','calendars_R_0','length').
assign('t537','t539').
load('t541','CalendarManager_R_?','populateLatestEvent_R_?').
actual('101','1','eventList_R_0').
callRet('101','eventList_R_0').
assign('t540','t542').
load('t544','CalendarManager_R_?','getCalendarFeed_R_?').
actual('102','1','calendarId_R_24').
callRet('102','calendarId_R_24').
assign('t543','t545').
assign('nextEvents_R_0','nextEvents_R_0').
assign('t546','isMultiCalendar_R_0').
load('t548','eventList_R_25','sort_R_?').
actual('103','1','sortByDate_R_?').
callRet('103','sortByDate_R_?').
assign('t547','t549').
load('t551','eventList_R_25','length').
assign('t550','t552').
load('t554','console','error_R_?').
actual('104','1','Error: feed retrieved, but no event found').
callRet('104','Error: feed retrieved, but no event found').
assign('t553','t555').
assign('nextEvent__R_0','null').
load('t557','canvasAnimation__R_0','drawFinal_R_?').
assign('t556','t558').
load('t559','eventList_R_25','0').
assign('nextEvent__R_0','t559').
load('t561','nextEvent__R_0','startTime').
load('t562','t561','setSeconds_R_?').
actual('106','1','0').
callRet('106','0').
actual('106','2','0').
callRet('106','0').
assign('t560','t563').
load('t565','nextEvents_R_0','push_R_?').
actual('107','1','nextEvent__R_0').
callRet('107','nextEvent__R_0').
assign('t564','t566').
load('t567','nextEvent__R_0','startTime').
assign('startTime_R_25','t567').
load('t568','eventList_R_25','i_R_25').
assign('event_R_?','t568').
assign('i_R_25','1').
assign('event_R_?','__undefined__').
assign('t570','i_R_25').
assign('t569','t570').
load('t571','event_R_?','startTime').
load('t572','t571','setSeconds').
actual('108','1','0').
callRet('108','0').
actual('108','2','0').
callRet('108','0').
assign('time_R_25','t573').
assign('t574','t575').
load('t577','nextEvents_R_0','push').
actual('109','1','event_R_?').
callRet('109','event_R_?').
assign('t576','t578').
assign('i_R_25','1').
assign('event_R_?','__undefined__').
load('t580','nextEvents_R_0','length').
assign('t579','t581').
load('t583','nextEvents_R_0','sort').
actual('110','1','sortByAuthor_R_?').
callRet('110','sortByAuthor_R_?').
assign('t582','t584').
load('t586','canvasAnimation__R_0','animate_R_?').
assign('t585','t587').
methodRet('d_lambda_R_25','null').
load('t588','DATE_TIME_REGEX_R_0','exec_R_?').
actual('112','1','rfc3339_R_26').
callRet('112','rfc3339_R_26').
assign('parts_R_26','t589').
assign('t591','parts_R_26').
assign('t590','t591').
load('t592','DATE_TIME_REGEX_Z_R_0','exec').
actual('113','1','rfc3339_R_26').
callRet('113','rfc3339_R_26').
assign('parts_R_26','t593').
load('t595','parts_R_26','length').
assign('t594','t597').
ptsTo('t598','n_Date_R_?_2').
prototype('n_Date_R_?_2', 'p_Date_R_?').
assign('d_R_26','t598').
load('t600','d_R_26','setUTCFullYear_R_?').
load('t601','parts_R_26','1').
load('t602','parts_R_26','2').
actual('114','1','t602').
callRet('114','t602').
actual('114','2','10').
callRet('114','10').
load('t605','parts_R_26','3').
actual('115','1','t601').
callRet('115','t601').
actual('115','2','t604').
callRet('115','t604').
actual('115','3','t605').
callRet('115','t605').
assign('t599','t606').
load('t608','d_R_26','setUTCHours_R_?').
load('t609','parts_R_26','4').
actual('116','1','t609').
callRet('116','t609').
assign('t607','t610').
load('t612','d_R_26','setUTCMinutes_R_?').
load('t613','parts_R_26','5').
actual('117','1','t613').
callRet('117','t613').
assign('t611','t614').
load('t616','d_R_26','setUTCSeconds_R_?').
load('t617','parts_R_26','6').
actual('118','1','t617').
callRet('118','t617').
assign('t615','t618').
assign('tzOffsetFeedMin_R_26','0').
load('t620','parts_R_26','length').
assign('t619','t621').
load('t622','parts_R_26','8').
actual('119','1','t622').
callRet('119','t622').
actual('119','2','10').
callRet('119','10').
load('t625','parts_R_26','9').
actual('120','1','t625').
callRet('120','t625').
actual('120','2','10').
callRet('120','10').
assign('tzOffsetFeedMin_R_26','t627').
load('t629','parts_R_26','7').
assign('t628','t630').
assign('t631','tzOffsetFeedMin_R_26').
assign('tzOffsetFeedMin_R_26','t631').
load('t632','d_R_26','getTime_R_?').
ptsTo('t637','n_Date_R_?_3').
prototype('n_Date_R_?_3', 'p_Date_R_?').
actual('122','1','t636').
callRet('122','t636').
methodRet('d_rfc3339StringToDate_R_26','t637').
load('t638','DATE_REGEX_R_0','exec').
actual('122','1','rfc3339_R_26').
callRet('122','rfc3339_R_26').
assign('parts_R_26','t639').
load('t641','parts_R_26','length').
assign('t640','t643').
load('t644','parts_R_26','1').
load('t645','parts_R_26','2').
actual('123','1','t645').
callRet('123','t645').
actual('123','2','10').
callRet('123','10').
load('t648','parts_R_26','3').
ptsTo('t649','n_Date_R_?_4').
prototype('n_Date_R_?_4', 'p_Date_R_?').
actual('124','1','t644').
callRet('124','t644').
actual('124','2','t647').
callRet('124','t647').
actual('124','3','t648').
callRet('124','t648').
methodRet('d_rfc3339StringToDate_R_26','t649').
methodRet('d_rfc3339StringToDate_R_26','null').
load('t650','event_1_R_27','startTime').
load('t651','t650','getTime_R_?').
load('t653','event_2_R_27','startTime').
load('t654','t653','getTime').
methodRet('d_sortByDate_R_27','t656').
assign('nameDiff_R_28','__undefined__').
load('t658','event_2_R_28','author').
assign('t657','t659').
methodRet('d_sortByAuthor_R_28','0').
assign('nameDiff_R_28','1').
methodRet('d_sortByAuthor_R_28','nameDiff_R_28').
assign('t660','nextEvent__R_0').
load('t661','nextEvent__R_0','startTime').
load('t662','t661','getTime_R_?').
assign('t_R_29','t665').
assign('t666','t667').
load('t669','CalendarManager_R_?','pollServer_R_?').
assign('t668','t670').
methodRet('d_redraw_R_29','null').
load('t672','canvasAnimation__R_0','animate_R_?').
assign('t671','t673').
assign('t674','t677').
load('t679','CalendarManager_R_?','pollServer').
assign('t678','t680').
ptsTo('t681','n_Date_R_?_5').
prototype('n_Date_R_?_5', 'p_Date_R_?').
load('t682','t681','getTime_R_?').
methodRet('d_getCurrentTime_R_30','t683').
assign('t684','data_R_31').
load('t685','data_R_31','replace_R_?').
actual('133','1','{"regexp": "&lt;", "modifiers": "g"}').
callRet('133','{"regexp": "&lt;", "modifiers": "g"}').
actual('133','2','<').
callRet('133','<').
assign('data_R_31','t686').
load('t687','data_R_31','replace').
actual('134','1','{"regexp": "&gt;", "modifiers": "g"}').
callRet('134','{"regexp": "&gt;", "modifiers": "g"}').
actual('134','2','>').
callRet('134','>').
assign('data_R_31','t688').
load('t689','data_R_31','replace').
actual('135','1','{"regexp": "&amp;", "modifiers": "g"}').
callRet('135','{"regexp": "&amp;", "modifiers": "g"}').
actual('135','2','&').
callRet('135','&').
assign('data_R_31','t690').
load('t691','data_R_31','replace').
actual('136','1','{"regexp": "%7B", "modifiers": "g"}').
callRet('136','{"regexp": "%7B", "modifiers": "g"}').
actual('136','2','{').
callRet('136','{').
assign('data_R_31','t692').
load('t693','data_R_31','replace').
actual('137','1','{"regexp": "%7D", "modifiers": "g"}').
callRet('137','{"regexp": "%7D", "modifiers": "g"}').
actual('137','2','}').
callRet('137','}').
assign('data_R_31','t694').
load('t695','data_R_31','replace').
actual('138','1','{"regexp": "&quot;", "modifiers": "g"}').
callRet('138','{"regexp": "&quot;", "modifiers": "g"}').
actual('138','2','"').
callRet('138','"').
assign('data_R_31','t696').
load('t697','data_R_31','replace').
actual('139','1','{"regexp": "&#39;", "modifiers": "g"}').
callRet('139','{"regexp": "&#39;", "modifiers": "g"}').
actual('139','2','"').
callRet('139','"').
assign('data_R_31','t698').
methodRet('d_filterSpecialChar_R_31','data_R_31').
load('t699','JSON_R_?','parse_R_?').
load('t700','localStorage_R_?','multiCalendar').
actual('140','1','t700').
callRet('140','t700').
assign('isMultiCalendar_R_0','t701').
load('t703','badgeAnimation__R_0','start_R_?').
assign('t702','t704').
load('t706','CalendarManager_R_?','pollServer_R_?').
assign('t705','t707').
load('t708','tab_R_33','url').
assign('url_R_33','t708').
assign('t710','url_R_33').
assign('t709','t710').
methodRet('d_onTabUpdated_R_33','null').
load('t712','url_R_33','indexOf').
actual('143','1','www.google.com/calendar/').
callRet('143','www.google.com/calendar/').
assign('t714','1').
load('t716','url_R_33','indexOf').
actual('144','1','www.google.com/a/').
callRet('144','www.google.com/a/').
assign('t718','1').
load('t720','url_R_33','lastIndexOf_R_?').
actual('145','1','/acs').
callRet('145','/acs').
load('t722','url_R_33','length').
load('t727','url_R_33','indexOf').
actual('146','1','www.google.com/accounts/').
callRet('146','www.google.com/accounts/').
assign('t729','1').
assign('t711','t731').
load('t733','url_R_33','indexOf_R_?').
actual('147','1','https://www.google.com/accounts/ServiceLogin?').
callRet('147','https://www.google.com/accounts/ServiceLogin?').
assign('t732','t735').
methodRet('d_onTabUpdated_R_33','null').
assign('t736','pendingLoadId__R_0').
actual('148','1','pendingLoadId__R_0').
callRet('148','pendingLoadId__R_0').
assign('t737','t738').
assign('pendingLoadId__R_0','null').
load('t739','CalendarManager_R_?','pollServer').
actual('149','1','t739').
callRet('149','t739').
actual('149','2','2000').
callRet('149','2000').
assign('pendingLoadId__R_0','t740').
load('t742','chrome','tabs').
load('t743','t742','getAllInWindow').
actual('150','1','null').
callRet('150','null').
actual('150','2','lambda_R_35').
callRet('150','lambda_R_35').
assign('t741','t744').
ptsTo('lambda_R_35','d_lambda_R_35').
heapPtsTo('d_lambda_R_35','prototype','p_lambda_R_35').
prototype('p_lambda_R_35','h_FP').
methodRet('d_lambda_R_35','lambda_R_35').
formal('d_lambda_R_35','1','tabs_R_35').
load('t745','tabs_R_35','i_R_35').
assign('tab_R_?','t745').
assign('i_R_35','0').
assign('tab_R_?','__undefined__').
assign('t747','i_R_35').
assign('t746','t747').
load('t749','tab_R_?','url').
load('t750','tab_R_?','url').
actual('151','1','t750').
callRet('151','t750').
assign('t748','t752').
load('t754','chrome','tabs').
load('t755','t754','update').
load('t756','tab_R_?','id').
store('t757','selected_R_?','true').
actual('152','1','t756').
callRet('152','t756').
actual('152','2','t757').
callRet('152','t757').
assign('t753','t758').
load('t760','CalendarManager_R_?','pollServer_R_?').
assign('t759','t761').
methodRet('d_lambda_R_35','null').
assign('i_R_35','0').
assign('tab_R_?','__undefined__').
load('t763','chrome','tabs').
load('t764','t763','create').
store('t765','url_R_?','GOOGLE_CALENDAR_URL_R_0').
actual('154','1','t765').
callRet('154','t765').
assign('t762','t766').
load('t768','CalendarManager_R_?','pollServer').
assign('t767','t769').
load('t770','url_R_36','indexOf_R_?').
actual('156','1','www.google.com/calendar').
callRet('156','www.google.com/calendar').
assign('t772','1').
assign('None','true').
methodRet('d_isCalendarUrl_R_36','false').
ptsTo('t774','n_BadgeAnimation_R_?_6').
prototype('n_BadgeAnimation_R_?_6', 'p_BadgeAnimation_R_?').
assign('badgeAnimation__R_0','t774').
ptsTo('t775','n_CanvasAnimation_R_?_7').
prototype('n_CanvasAnimation_R_?_7', 'p_CanvasAnimation_R_?').
assign('canvasAnimation__R_0','t775').
load('t776','JSON_R_?','parse_R_?').
load('t777','localStorage_R_?','multiCalendar').
actual('157','1','t778').
callRet('157','t778').
assign('isMultiCalendar_R_0','t779').
load('t781','chrome','browserAction').
load('t782','t781','setIcon').
store('t783','path_R_?','../images/icon-16.gif').
actual('158','1','t783').
callRet('158','t783').
assign('t780','t784').
load('t786','badgeAnimation__R_0','start_R_?').
assign('t785','t787').
load('t789','CalendarManager_R_?','pollServer_R_?').
assign('t788','t790').
load('t792','window','setInterval_R_?').
actual('161','1','redraw_R_?').
callRet('161','redraw_R_?').
actual('161','2','DRAW_INTERVAL_R_0').
callRet('161','DRAW_INTERVAL_R_0').
assign('t791','t793').
load('t795','chrome','tabs').
load('t796','t795','onUpdated').
load('t797','t796','addListener').
actual('162','1','onTabUpdated_R_?').
callRet('162','onTabUpdated_R_?').
assign('t794','t798').
load('t800','chrome','browserAction').
load('t801','t800','onClicked').
load('t802','t801','addListener').
actual('163','1','lambda_R_38').
callRet('163','lambda_R_38').
assign('t799','t803').
ptsTo('lambda_R_38','d_lambda_R_38').
heapPtsTo('d_lambda_R_38','prototype','p_lambda_R_38').
prototype('p_lambda_R_38','h_FP').
methodRet('d_lambda_R_38','lambda_R_38').
formal('d_lambda_R_38','1','tab_R_38').
assign('t804','t805').
load('t806','JSON_R_?','parse_R_?').
load('t807','localStorage_R_?','multiCalendar').
actual('165','1','t808').
callRet('165','t808').
assign('isMultiCalendar_R_0','t809').
load('t810','$','multiCalendar').
load('t811','t810','checked').
store('t810','checked','isMultiCalendar_R_0').
load('t812','$','multiCalendarText').
load('t813','t812','innerHTML').
load('t814','chrome','i18n').
load('t815','t814','getMessage').
actual('166','1','multiCalendarText').
callRet('166','multiCalendarText').
store('t812','innerHTML','t816').
load('t817','$','optionsTitle').
load('t818','t817','innerHTML').
load('t819','chrome','i18n').
load('t820','t819','getMessage').
actual('167','1','optionsTitle').
callRet('167','optionsTitle').
store('t817','innerHTML','t821').
load('t822','$','imageTooltip').
load('t823','t822','title').
load('t824','chrome','i18n').
load('t825','t824','getMessage').
actual('168','1','imageTooltip').
callRet('168','imageTooltip').
store('t822','title','t826').
load('t827','$','imageTooltip').
load('t828','t827','alt').
load('t829','chrome','i18n').
load('t830','t829','getMessage').
actual('169','1','imageTooltip').
callRet('169','imageTooltip').
store('t827','alt','t831').
load('t832','$','multiCalendarText').
load('t833','t832','title').
load('t834','chrome','i18n').
load('t835','t834','getMessage').
actual('170','1','multiCalendarToolTip').
callRet('170','multiCalendarToolTip').
store('t832','title','t836').
load('t837','$','multiCalendar').
load('t838','t837','title').
load('t839','chrome','i18n').
load('t840','t839','getMessage').
actual('171','1','multiCalendarToolTip').
callRet('171','multiCalendarToolTip').
store('t837','title','t841').
load('t842','$','extensionName').
load('t843','t842','innerHTML').
load('t844','chrome','i18n').
load('t845','t844','getMessage').
actual('172','1','extensionName').
callRet('172','extensionName').
store('t842','innerHTML','t846').
load('t848','chrome','i18n').
load('t849','t848','getMessage').
actual('173','1','direction').
callRet('173','direction').
assign('t847','t851').
load('t852','document','querySelector_R_?').
load('t853','t852','body').
load('t854','t853','style').
load('t855','t854','direction').
store('t854','direction','rtl').
actual('174','1','multiCalendar').
callRet('174','multiCalendar').
assign('multiCalendarId_R_40','t856').
load('t857','localStorage_R_?','multiCalendar').
load('t858','multiCalendarId_R_40','checked').
store('localStorage_R_?','multiCalendar','t858').
assign('t859','multiCalendarId_R_40').
load('t860','multiCalendar_R_?','disabled').
store('multiCalendar_R_?','disabled','true').
load('t861','$','status').
load('t862','t861','innerHTML').
load('t863','chrome','i18n').
load('t864','t863','getMessage').
actual('175','1','status_saving').
callRet('175','status_saving').
store('t861','innerHTML','t865').
load('t866','$','status').
load('t867','t866','style').
load('t868','t867','display').
store('t867','display','block').
load('t870','chrome','extension').
load('t871','t870','getBackgroundPage').
load('t873','t872','onSettingsChange').
assign('t869','t874').
load('t876','request_R_41','message').
assign('t877','t876').
assign('t875','t877').
methodRet('d_lambda_R_41','null').
load('t878','document','getElementById_R_?').
actual('178','1','id_R_42').
callRet('178','id_R_42').
methodRet('d_$_R_42','t879').
load('t880','document','getElementById_R_?').
actual('179','1','multiCalendar').
callRet('179','multiCalendar').
assign('temp_R_43','t881').
load('t883','temp_R_43','addEventListener_R_?').
actual('180','1','click').
callRet('180','click').
actual('180','2','lambda_R_44').
callRet('180','lambda_R_44').
assign('t882','t884').
ptsTo('lambda_R_44','d_lambda_R_44').
heapPtsTo('d_lambda_R_44','prototype','p_lambda_R_44').
prototype('p_lambda_R_44','h_FP').
methodRet('d_lambda_R_44','lambda_R_44').
formal('d_lambda_R_44','1','event_R_44').
assign('t885','t886').
%% basicrules.pl
%% This file contains the inference rules needed for the points-to analysis
%% using Datalog. These rules will be embedded into the .pl file generated
%% from the JavaScript file/extension.

%% FIXME: style_check directive not available in Datalog 
%%:- style_check(-discontiguous).

:- dynamic
        alloc/2,
        load/3,
        store/3,
        assign/2,
        directHeapStoresTo/3,
        directHeapPointsTo/3,
        actual/3.

%% Basic Rules
ptsTo(V,H) :-
        alloc(V,H).
ptsTo(V_1,H) :-
        assign(V_1,V_2),
        ptsTo(V_2,H).

directHeapStoresTo(H_1,F,H_2) :-
        store(V_1,F,V_2),
        ptsTo(V_1,H_1),        
        ptsTo(V_2,H_2).
directHeapPointsTo(H_1,F,H_2) :-        
        directHeapStoresTo(H_1,F,H_2).
ptsTo(V_2,H_2) :-        
        load(V_2,V_1,F),         
        ptsTo(V_1,H_1),        
        heapPtsTo(H_1,F,H_2).
heapPtsTo(H_1,F,H_2) :-        
        directHeapPointsTo(H_1,F,H_2).

%% Call graph
calls(I,M) :-        
        actual(I,0,C),        
        ptsTo(C,M).

%% Interprocedural assignments
assign(V_1,V_2) :-        
        calls(I,M),        
        formal(M,Z,V_1),        
        actual(I,Z,V_2).
assign(V_2,V_1) :-        
        calls(I,M),        
        methodRet(M,V_1),        
        callRet(I,V_2).

%% Prototype handling
heapPtsTo(H_1,F,H_2) :-        
        prototype(H_1,H),        
        heapPtsTo(H,F,H_2).

%%% chrome.pl %%%
% A hard-coded model of the Google Chrome API to be used by datalog.py. 
% datalog.py will load this file and embed it to the .pl file generated
% by the JavaScript files. This file is used to simulate the existence
% of the Chrome API in the environment.
%
% Change Logs
% -    Implemented all chrome API methods.
%
% To-Dos
% -    Implement other chrome API events!


% chrome object
ptsTo('chrome', 'd_chrome').
heapPtsTo('d_chrome', 'prototype', 'p_chrome').

% chrome.tabs object
heapPtsTo('d_chrome', 'tabs', 'd_chrome_tabs').

% chrome.tabs.* methods 
heapPtsTo('d_chrome_tabs', 'captureVisibleTab', 'd_chrome_tabs_captureVisibleTab').
heapPtsTo('d_chrome_tabs_captureVisibleTab', 'prototype', 'p_chrome_tabs_captureVisibleTab').
prototype('p_chrome_tabs_captureVisibleTab', 'h_FP').
heapPtsTo('d_chrome_tabs', 'connect', 'd_chrome_tabs_connect').
heapPtsTo('d_chrome_tabs_connect', 'prototype', 'p_chrome_tabs_connect').
prototype('p_chrome_tabs_connect', 'h_FP').
heapPtsTo('d_chrome_tabs', 'create', 'd_chrome_tabs_create').
heapPtsTo('d_chrome_tabs_create', 'prototype', 'p_chrome_tabs_create').
prototype('p_chrome_tabs_create', 'h_FP').
heapPtsTo('d_chrome_tabs', 'detectLanguage', 'd_chrome_tabs_detectLanguage').
heapPtsTo('d_chrome_tabs_detectLanguage', 'prototype', 'p_chrome_tabs_detectLanguage').
prototype('p_chrome_tabs_detectLanguage', 'h_FP').
heapPtsTo('d_chrome_tabs', 'executeScript', 'd_chrome_tabs_executeScript').
heapPtsTo('d_chrome_tabs_executeScript', 'prototype', 'p_chrome_tabs_executeScript').
prototype('p_chrome_tabs_executeScript', 'h_FP').
heapPtsTo('d_chrome_tabs', 'get', 'd_chrome_tabs_get').
heapPtsTo('d_chrome_tabs_get', 'prototype', 'p_chrome_tabs_get').
prototype('p_chrome_tabs_get', 'h_FP').
heapPtsTo('d_chrome_tabs', 'getCurrent', 'd_chrome_tabs_getCurrent').
heapPtsTo('d_chrome_tabs_getCurrent', 'prototype', 'p_chrome_tabs_getCurrent').
prototype('p_chrome_tabs_getCurrent', 'h_FP').
heapPtsTo('d_chrome_tabs', 'highlight', 'd_chrome_tabs_highlight').
heapPtsTo('d_chrome_tabs_highlight', 'prototype', 'p_chrome_tabs_highlight').
prototype('p_chrome_tabs_highlight', 'h_FP').
heapPtsTo('d_chrome_tabs', 'insertCSS', 'd_chrome_tabs_insertCSS').
heapPtsTo('d_chrome_tabs_insertCSS', 'prototype', 'p_chrome_tabs_insertCSS').
prototype('p_chrome_tabs_insertCSS', 'h_FP').
heapPtsTo('d_chrome_tabs', 'move', 'd_chrome_tabs_move').
heapPtsTo('d_chrome_tabs_move', 'prototype', 'p_chrome_tabs_move').
prototype('p_chrome_tabs_move', 'h_FP').
heapPtsTo('d_chrome_tabs', 'query', 'd_chrome_tabs_query').
heapPtsTo('d_chrome_tabs_query', 'prototype', 'p_chrome_tabs_query').
prototype('p_chrome_tabs_query', 'h_FP').
heapPtsTo('d_chrome_tabs', 'reload', 'd_chrome_tabs_reload').
heapPtsTo('d_chrome_tabs_reload', 'prototype', 'p_chrome_tabs_reload').
prototype('p_chrome_tabs_reload', 'h_FP').
heapPtsTo('d_chrome_tabs', 'remove', 'd_chrome_tabs_remove').
heapPtsTo('d_chrome_tabs_remove', 'prototype', 'p_chrome_tabs_remove').
prototype('p_chrome_tabs_remove', 'h_FP').
heapPtsTo('d_chrome_tabs', 'sendRequest', 'd_chrome_tabs_sendRequest').
heapPtsTo('d_chrome_tabs_sendRequest', 'prototype', 'p_chrome_tabs_sendRequest').
prototype('p_chrome_tabs_sendRequest', 'h_FP').
heapPtsTo('d_chrome_tabs', 'update', 'd_chrome_tabs_update').
heapPtsTo('d_chrome_tabs_update', 'prototype', 'p_chrome_tabs_update').
prototype('p_chrome_tabs_update', 'h_FP').

% chrome.bookmarks object
heapPtsTo('d_chrome', 'bookmarks', 'd_chrome_bookmarks').

% chrome.bookmarks.* methods
heapPtsTo('d_chrome_bookmarks', 'create', 'd_chrome_bookmarks_create').
heapPtsTo('d_chrome_bookmarks_create', 'prototype', 'p_chrome_bookmarks_create').
prototype('p_chrome_bookmarks_create', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'get', 'd_chrome_bookmarks_get').
heapPtsTo('d_chrome_bookmarks_get', 'prototype', 'p_chrome_bookmarks_get').
prototype('p_chrome_bookmarks_get', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'getChildren', 'd_chrome_bookmarks_getChildren').
heapPtsTo('d_chrome_bookmarks_getChildren', 'prototype', 'p_chrome_bookmarks_getChildren').
prototype('p_chrome_bookmarks_getChildren', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'getRecent', 'd_chrome_bookmarks_getRecent').
heapPtsTo('d_chrome_bookmarks_getRecent', 'prototype', 'p_chrome_bookmarks_getRecent').
prototype('p_chrome_bookmarks_getRecent', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'getSubTree', 'd_chrome_bookmarks_getSubTree').
heapPtsTo('d_chrome_bookmarks_getSubTree', 'prototype', 'p_chrome_bookmarks_getSubTree').
prototype('p_chrome_bookmarks_getSubTree', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'getTree', 'd_chrome_bookmarks_getTree').
heapPtsTo('d_chrome_bookmarks_getTree', 'prototype', 'p_chrome_bookmarks_getTree').
prototype('p_chrome_bookmarks_getTree', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'move', 'd_chrome_bookmarks_move').
heapPtsTo('d_chrome_bookmarks_move', 'prototype', 'p_chrome_bookmarks_move').
prototype('p_chrome_bookmarks_move', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'remove', 'd_chrome_bookmarks_remove').
heapPtsTo('d_chrome_bookmarks_remove', 'prototype', 'p_chrome_bookmarks_remove').
prototype('p_chrome_bookmarks_remove', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'removeTree', 'd_chrome_bookmarks_removeTree').
heapPtsTo('d_chrome_bookmarks_removeTree', 'prototype', 'p_chrome_bookmarks_removeTree').
prototype('p_chrome_bookmarks_removeTree', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'search', 'd_chrome_bookmarks_search').
heapPtsTo('d_chrome_bookmarks_search', 'prototype', 'p_chrome_bookmarks_search').
prototype('p_chrome_bookmarks_search', 'h_FP').
heapPtsTo('d_chrome_bookmarks', 'update', 'd_chrome_bookmarks_update').
heapPtsTo('d_chrome_bookmarks_update', 'prototype', 'p_chrome_bookmarks_update').
prototype('p_chrome_bookmarks_update', 'h_FP').

% chrome.extension object
heapPtsTo('d_chrome', 'extension', 'd_chrome_extension').

% chrome.extension.* methods
heapPtsTo('d_chrome_extension', 'connect', 'd_chrome_extension_connect').
heapPtsTo('d_chrome_extension_connect', 'prototype', 'p_chrome_extension_connect').
prototype('p_chrome_extension_connect', 'h_FP').
heapPtsTo('d_chrome_extension', 'getBackgroundPage', 'd_chrome_extension_getBackgroundPage').
heapPtsTo('d_chrome_extension_getBackgroundPage', 'prototype', 'p_chrome_extension_getBackgroundPage').
prototype('p_chrome_extension_getBackgroundPage', 'h_FP').
heapPtsTo('d_chrome_extension', 'getURL', 'd_chrome_extension_getURL').
heapPtsTo('d_chrome_extension_getURL', 'prototype', 'p_chrome_extension_getURL').
prototype('p_chrome_extension_getURL', 'h_FP').
heapPtsTo('d_chrome_extension', 'getViews', 'd_chrome_extension_getViews').
heapPtsTo('d_chrome_extension_getViews', 'prototype', 'p_chrome_extension_getViews').
prototype('p_chrome_extension_getViews', 'h_FP').
heapPtsTo('d_chrome_extension', 'isAllowedFileSchemeAccess', 'd_chrome_extension_isAllowedFileSchemeAccess').
heapPtsTo('d_chrome_extension_isAllowedFileSchemeAccess', 'prototype', 'p_chrome_extension_isAllowedFileSchemeAccess').
prototype('p_chrome_extension_isAllowedFileSchemeAccess', 'h_FP').
heapPtsTo('d_chrome_extension', 'isAllowedIncognitoAccess', 'd_chrome_extension_isAllowedIncognitoAccess').
heapPtsTo('d_chrome_extension_isAllowedIncognitoAccess', 'prototype', 'p_chrome_extension_isAllowedIncognitoAccess').
prototype('p_chrome_extension_isAllowedIncognitoAccess', 'h_FP').
heapPtsTo('d_chrome_extension', 'sendRequest', 'd_chrome_extension_sendRequest').
heapPtsTo('d_chrome_extension_sendRequest', 'prototype', 'p_chrome_extension_sendRequest').
prototype('p_chrome_extension_sendRequest', 'h_FP').
heapPtsTo('d_chrome_extension', 'setUpdateUrlData', 'd_chrome_extension_setUpdateUrlData').
heapPtsTo('d_chrome_extension_setUpdateUrlData', 'prototype', 'p_chrome_extension_setUpdateUrlData').
prototype('p_chrome_extension_setUpdateUrlData', 'h_FP').

% chrome.browserAction object
heapPtsTo('d_chrome', 'browserAction', 'd_chrome_browserAction').

% chrome.browserAction.* methods
heapPtsTo('d_chrome_browserAction', 'setBadgeBackgroundColor', 'd_chrome_browserAction_setBadgeBackgroundColor').
heapPtsTo('d_chrome_browserAction_setBadgeBackgroundColor', 'prototype', 'p_chrome_browserAction_setBadgeBackgroundColor').
prototype('p_chrome_browserAction_setBadgeBackgroundColor', 'h_FP').
heapPtsTo('d_chrome_browserAction', 'setBadgeText', 'd_chrome_browserAction_setBadgeText').
heapPtsTo('d_chrome_browserAction_setBadgeText', 'prototype', 'p_chrome_browserAction_setBadgeText').
prototype('p_chrome_browserAction_setBadgeText', 'h_FP').
heapPtsTo('d_chrome_browserAction', 'setIcon', 'd_chrome_browserAction_setIcon').
heapPtsTo('d_chrome_browserAction_setIcon', 'prototype', 'p_chrome_browserAction_setIcon').
prototype('p_chrome_browserAction_setIcon', 'h_FP').
heapPtsTo('d_chrome_browserAction', 'setPopup', 'd_chrome_browserAction_setPopup').
heapPtsTo('d_chrome_browserAction_setPopup', 'prototype', 'p_chrome_browserAction_setPopup').
prototype('p_chrome_browserAction_setPopup', 'h_FP').
heapPtsTo('d_chrome_browserAction', 'setTitle', 'd_chrome_browserAction_setTitle').
heapPtsTo('d_chrome_browserAction_setTitle', 'prototype', 'p_chrome_browserAction_setTitle').
prototype('p_chrome_browserAction_setTitle', 'h_FP').

% chrome.contentSettings object
% Note: Doesn't have any methods
heapPtsTo('d_chrome', 'contentSettings', 'd_chrome_contentSettings').

% chrome.contextMenus object
heapPtsTo('d_chrome', 'contextMenus', 'd_chrome_contextMenus').

% chrome.contextMenus.* methods
heapPtsTo('d_chrome_contextMenus', 'create', 'd_chrome_contextMenus_create').
heapPtsTo('d_chrome_contextMenus_create', 'prototype', 'p_chrome_contextMenus_create').
prototype('p_chrome_contextMenus_create', 'h_FP').
heapPtsTo('d_chrome_contextMenus', 'remove', 'd_chrome_contextMenus_remove').
heapPtsTo('d_chrome_contextMenus_remove', 'prototype', 'p_chrome_contextMenus_remove').
prototype('p_chrome_contextMenus_remove', 'h_FP').
heapPtsTo('d_chrome_contextMenus', 'removeAll', 'd_chrome_contextMenus_removeAll').
heapPtsTo('d_chrome_contextMenus_removeAll', 'prototype', 'p_chrome_contextMenus_removeAll').
prototype('p_chrome_contextMenus_removeAll', 'h_FP').
heapPtsTo('d_chrome_contextMenus', 'update', 'd_chrome_contextMenus_update').
heapPtsTo('d_chrome_contextMenus_update', 'prototype', 'p_chrome_contextMenus_update').
prototype('p_chrome_contextMenus_update', 'h_FP').

% chrome.cookies object
heapPtsTo('d_chrome', 'cookies', 'd_chrome_cookies').

% chrome.cookies.* methods
heapPtsTo('d_chrome_cookies', 'get', 'd_chrome_cookies_get').
heapPtsTo('d_chrome_cookies_get', 'prototype', 'p_chrome_cookies_get').
prototype('p_chrome_cookies_get', 'h_FP').
heapPtsTo('d_chrome_cookies', 'getAll', 'd_chrome_cookies_getAll').
heapPtsTo('d_chrome_cookies_getAll', 'prototype', 'p_chrome_cookies_getAll').
prototype('p_chrome_cookies_getAll', 'h_FP').
heapPtsTo('d_chrome_cookies', 'getAllCookieStores', 'd_chrome_cookies_getAllCookieStores').
heapPtsTo('d_chrome_cookies_getAllCookieStores', 'prototype', 'p_chrome_cookies_getAllCookieStores').
prototype('p_chrome_cookies_getAllCookieStores', 'h_FP').
heapPtsTo('d_chrome_cookies', 'remove', 'd_chrome_cookies_remove').
heapPtsTo('d_chrome_cookies_remove', 'prototype', 'p_chrome_cookies_remove').
prototype('p_chrome_cookies_remove', 'h_FP').
heapPtsTo('d_chrome_cookies', 'set', 'd_chrome_cookies_set').
heapPtsTo('d_chrome_cookies_set', 'prototype', 'p_chrome_cookies_set').
prototype('p_chrome_cookies_set', 'h_FP').

% chrome.debugger object
heapPtsTo('d_chrome', 'debugger', 'd_chrome_debugger').

% chrome.debugger.* methods
heapPtsTo('d_chrome_debugger', 'attach', 'd_chrome_debugger_attach').
heapPtsTo('d_chrome_debugger_attach', 'prototype', 'p_chrome_debugger_attach').
prototype('p_chrome_debugger_attach', 'h_FP').
heapPtsTo('d_chrome_debugger', 'detach', 'd_chrome_debugger_detach').
heapPtsTo('d_chrome_debugger_detach', 'prototype', 'p_chrome_debugger_detach').
prototype('p_chrome_debugger_detach', 'h_FP').
heapPtsTo('d_chrome_debugger', 'sendCommand', 'd_chrome_debugger_sendCommand').
heapPtsTo('d_chrome_debugger_sendCommand', 'prototype', 'p_chrome_debugger_sendCommand').
prototype('p_chrome_debugger_sendCommand', 'h_FP').

% chrome.fileBrowserHandler object
% Note: Doesn't have any methods
heapPtsTo('d_chrome', 'fileBrowserHandler', 'd_chrome_fileBrowserHandler').

% chrome.history object
heapPtsTo('d_chrome', 'history', 'd_chrome_history').

% chrome.history.* methods
heapPtsTo('d_chrome_history', 'addUrl', 'd_chrome_history_addUrl').
heapPtsTo('d_chrome_history_addUrl', 'prototype', 'p_chrome_history_addUrl').
prototype('p_chrome_history_addUrl', 'h_FP').
heapPtsTo('d_chrome_history', 'deleteAll', 'd_chrome_history_deleteAll').
heapPtsTo('d_chrome_history_deleteAll', 'prototype', 'p_chrome_history_deleteAll').
prototype('p_chrome_history_deleteAll', 'h_FP').
heapPtsTo('d_chrome_history', 'deleteRange', 'd_chrome_history_deleteRange').
heapPtsTo('d_chrome_history_deleteRange', 'prototype', 'p_chrome_history_deleteRange').
prototype('p_chrome_history_deleteRange', 'h_FP').
heapPtsTo('d_chrome_history', 'deleteUrl', 'd_chrome_history_deleteUrl').
heapPtsTo('d_chrome_history_deleteUrl', 'prototype', 'p_chrome_history_deleteUrl').
prototype('p_chrome_history_deleteUrl', 'h_FP').
heapPtsTo('d_chrome_history', 'getVisits', 'd_chrome_history_getVisits').
heapPtsTo('d_chrome_history_getVisits', 'prototype', 'p_chrome_history_getVisits').
prototype('p_chrome_history_getVisits', 'h_FP').
heapPtsTo('d_chrome_history', 'search', 'd_chrome_history_search').
heapPtsTo('d_chrome_history_search', 'prototype', 'p_chrome_history_search').
prototype('p_chrome_history_search', 'h_FP').

% chrome.i18n object
heapPtsTo('d_chrome', 'i18n', 'd_chrome_i18n').

% chrome.i18n.* methods
heapPtsTo('d_chrome_i18n', 'getAcceptLanguages', 'd_chrome_i18n_getAcceptLanguages').
heapPtsTo('d_chrome_i18n_getAcceptLanguages', 'prototype', 'p_chrome_i18n_getAcceptLanguages').
prototype('p_chrome_i18n_getAcceptLanguages', 'h_FP').
heapPtsTo('d_chrome_i18n', 'getMessage', 'd_chrome_i18n_getMessage').
heapPtsTo('d_chrome_i18n_getMessage', 'prototype', 'p_chrome_i18n_getMessage').
prototype('p_chrome_i18n_getMessage', 'h_FP').

% chrome.idle object
heapPtsTo('d_chrome', 'idle', 'd_chrome_idle').

% chrome.idle.* methods
heapPtsTo('d_chrome_idle', 'queryState', 'd_chrome_idle_queryState').
heapPtsTo('d_chrome_idle_queryState', 'prototype', 'p_chrome_idle_queryState').
prototype('p_chrome_idle_queryState', 'h_FP').

% chrome.input.ime object
heapPtsTo('d_chrome', 'input.ime', 'd_chrome_input').

% chrome.input.ime.* methods
heapPtsTo('d_chrome_input', 'clearComposition', 'd_chrome_input_clearComposition').
heapPtsTo('d_chrome_input_clearComposition', 'prototype', 'p_chrome_input_clearComposition').
prototype('p_chrome_input_clearComposition', 'h_FP').
heapPtsTo('d_chrome_input', 'commitText', 'd_chrome_input_commitText').
heapPtsTo('d_chrome_input_commitText', 'prototype', 'p_chrome_input_commitText').
prototype('p_chrome_input_commitText', 'h_FP').
heapPtsTo('d_chrome_input', 'setCandidateWindowProperties', 'd_chrome_input_setCandidateWindowProperties').
heapPtsTo('d_chrome_input_setCandidateWindowProperties', 'prototype', 'p_chrome_input_setCandidateWindowProperties').
prototype('p_chrome_input_setCandidateWindowProperties', 'h_FP').
heapPtsTo('d_chrome_input', 'setCandidates', 'd_chrome_input_setCandidates').
heapPtsTo('d_chrome_input_setCandidates', 'prototype', 'p_chrome_input_setCandidates').
prototype('p_chrome_input_setCandidates', 'h_FP').
heapPtsTo('d_chrome_input', 'setComposition', 'd_chrome_input_setComposition').
heapPtsTo('d_chrome_input_setComposition', 'prototype', 'p_chrome_input_setComposition').
prototype('p_chrome_input_setComposition', 'h_FP').
heapPtsTo('d_chrome_input', 'setCursorPosition', 'd_chrome_input_setCursorPosition').
heapPtsTo('d_chrome_input_setCursorPosition', 'prototype', 'p_chrome_input_setCursorPosition').
prototype('p_chrome_input_setCursorPosition', 'h_FP').
heapPtsTo('d_chrome_input', 'setMenuItems', 'd_chrome_input_setMenuItems').
heapPtsTo('d_chrome_input_setMenuItems', 'prototype', 'p_chrome_input_setMenuItems').
prototype('p_chrome_input_setMenuItems', 'h_FP').
heapPtsTo('d_chrome_input', 'updateMenuItems', 'd_chrome_input_updateMenuItems').
heapPtsTo('d_chrome_input_updateMenuItems', 'prototype', 'p_chrome_input_updateMenuItems').
prototype('p_chrome_input_updateMenuItems', 'h_FP').

% chrome.management object
heapPtsTo('d_chrome', 'management', 'd_chrome_management').

% chrome.management.* methods
heapPtsTo('d_chrome_management', 'get', 'd_chrome_management_get').
heapPtsTo('d_chrome_management_get', 'prototype', 'p_chrome_management_get').
prototype('p_chrome_management_get', 'h_FP').
heapPtsTo('d_chrome_management', 'getAll', 'd_chrome_management_getAll').
heapPtsTo('d_chrome_management_getAll', 'prototype', 'p_chrome_management_getAll').
prototype('p_chrome_management_getAll', 'h_FP').
heapPtsTo('d_chrome_management', 'getPermissionWarningsById', 'd_chrome_management_getPermissionWarningsById').
heapPtsTo('d_chrome_management_getPermissionWarningsById', 'prototype', 'p_chrome_management_getPermissionWarningsById').
prototype('p_chrome_management_getPermissionWarningsById', 'h_FP').
heapPtsTo('d_chrome_management', 'getPermissionWarningsByManifest', 'd_chrome_management_getPermissionWarningsByManifest').
heapPtsTo('d_chrome_management_getPermissionWarningsByManifest', 'prototype', 'p_chrome_management_getPermissionWarningsByManifest').
prototype('p_chrome_management_getPermissionWarningsByManifest', 'h_FP').
heapPtsTo('d_chrome_management', 'launchApp', 'd_chrome_management_launchApp').
heapPtsTo('d_chrome_management_launchApp', 'prototype', 'p_chrome_management_launchApp').
prototype('p_chrome_management_launchApp', 'h_FP').
heapPtsTo('d_chrome_management', 'setEnabled', 'd_chrome_management_setEnabled').
heapPtsTo('d_chrome_management_setEnabled', 'prototype', 'p_chrome_management_setEnabled').
prototype('p_chrome_management_setEnabled', 'h_FP').
heapPtsTo('d_chrome_management', 'uninstall', 'd_chrome_management_uninstall').
heapPtsTo('d_chrome_management_uninstall', 'prototype', 'p_chrome_management_uninstall').
prototype('p_chrome_management_uninstall', 'h_FP').

% chrome.omnibox object
heapPtsTo('d_chrome', 'omnibox', 'd_chrome_omnibox').

% chrome.omnibox.* methods
heapPtsTo('d_chrome_omnibox', 'setDefaultSuggestion', 'd_chrome_omnibox_setDefaultSuggestion').
heapPtsTo('d_chrome_omnibox_setDefaultSuggestion', 'prototype', 'p_chrome_omnibox_setDefaultSuggestion').
prototype('p_chrome_omnibox_setDefaultSuggestion', 'h_FP').

% chrome.pageAction object
heapPtsTo('d_chrome', 'pageAction', 'd_chrome_pageAction').

% chrome.pageAction.* methods
heapPtsTo('d_chrome_pageAction', 'hide', 'd_chrome_pageAction_hide').
heapPtsTo('d_chrome_pageAction_hide', 'prototype', 'p_chrome_pageAction_hide').
prototype('p_chrome_pageAction_hide', 'h_FP').
heapPtsTo('d_chrome_pageAction', 'setIcon', 'd_chrome_pageAction_setIcon').
heapPtsTo('d_chrome_pageAction_setIcon', 'prototype', 'p_chrome_pageAction_setIcon').
prototype('p_chrome_pageAction_setIcon', 'h_FP').
heapPtsTo('d_chrome_pageAction', 'setPopup', 'd_chrome_pageAction_setPopup').
heapPtsTo('d_chrome_pageAction_setPopup', 'prototype', 'p_chrome_pageAction_setPopup').
prototype('p_chrome_pageAction_setPopup', 'h_FP').
heapPtsTo('d_chrome_pageAction', 'setTitle', 'd_chrome_pageAction_setTitle').
heapPtsTo('d_chrome_pageAction_setTitle', 'prototype', 'p_chrome_pageAction_setTitle').
prototype('p_chrome_pageAction_setTitle', 'h_FP').
heapPtsTo('d_chrome_pageAction', 'show', 'd_chrome_pageAction_show').
heapPtsTo('d_chrome_pageAction_show', 'prototype', 'p_chrome_pageAction_show').
prototype('p_chrome_pageAction_show', 'h_FP').


% chrome.pageCapture object
heapPtsTo('d_chrome', 'pageCapture', 'd_chrome_pageCapture').

% chrome.pageCapture.* methods
heapPtsTo('d_chrome_pageCapture', 'saveAsMHTML', 'd_chrome_pageCapture_saveAsMHTML').
heapPtsTo('d_chrome_pageCapture_saveAsMHTML', 'prototype', 'p_chrome_pageCapture_saveAsMHTML').
prototype('p_chrome_pageCapture_saveAsMHTML', 'h_FP').

% chrome.permissions object
heapPtsTo('d_chrome', 'permissions', 'd_chrome_permissions').

% chrome.permissions.* method
heapPtsTo('d_chrome_permissions', 'contains', 'd_chrome_permissions_contains').
heapPtsTo('d_chrome_permissions_contains', 'prototype', 'p_chrome_permissions_contains').
prototype('p_chrome_permissions_contains', 'h_FP').
heapPtsTo('d_chrome_permissions', 'getAll', 'd_chrome_permissions_getAll').
heapPtsTo('d_chrome_permissions_getAll', 'prototype', 'p_chrome_permissions_getAll').
prototype('p_chrome_permissions_getAll', 'h_FP').
heapPtsTo('d_chrome_permissions', 'remove', 'd_chrome_permissions_remove').
heapPtsTo('d_chrome_permissions_remove', 'prototype', 'p_chrome_permissions_remove').
prototype('p_chrome_permissions_remove', 'h_FP').
heapPtsTo('d_chrome_permissions', 'request', 'd_chrome_permissions_request').
heapPtsTo('d_chrome_permissions_request', 'prototype', 'p_chrome_permissions_request').
prototype('p_chrome_permissions_request', 'h_FP').


% chrome.privacy object
% Note: Doesn't have any methods. 
heapPtsTo('d_chrome', 'privacy', 'd_chrome_privacy').

% chrome.proxy object
% Note: Doesn't have any methods. 
heapPtsTo('d_chrome', 'proxy', 'd_chrome_proxy').

% chrome.tts object
heapPtsTo('d_chrome', 'tts', 'd_chrome_tts').

% chrome.tts.* methods
heapPtsTo('d_chrome_tts', 'getVoices', 'd_chrome_tts_getVoices').
heapPtsTo('d_chrome_tts_getVoices', 'prototype', 'p_chrome_tts_getVoices').
prototype('p_chrome_tts_getVoices', 'h_FP').
heapPtsTo('d_chrome_tts', 'isSpeaking', 'd_chrome_tts_isSpeaking').
heapPtsTo('d_chrome_tts_isSpeaking', 'prototype', 'p_chrome_tts_isSpeaking').
prototype('p_chrome_tts_isSpeaking', 'h_FP').
heapPtsTo('d_chrome_tts', 'speak', 'd_chrome_tts_speak').
heapPtsTo('d_chrome_tts_speak', 'prototype', 'p_chrome_tts_speak').
prototype('p_chrome_tts_speak', 'h_FP').
heapPtsTo('d_chrome_tts', 'stop', 'd_chrome_tts_stop').
heapPtsTo('d_chrome_tts_stop', 'prototype', 'p_chrome_tts_stop').
prototype('p_chrome_tts_stop', 'h_FP').

% chrome.ttsEngine object
% Note: Doesn't have any methods.
heapPtsTo('d_chrome', 'ttsEngine', 'd_chrome_ttsEngine').

% chrome.types object
heapPtsTo('d_chrome', 'types', 'd_chrome_types').

% chrome.types.* method
heapPtsTo('d_chrome_types', 'clear', 'd_chrome_types_clear').
heapPtsTo('d_chrome_types_clear', 'prototype', 'p_chrome_types_clear').
prototype('p_chrome_types_clear', 'h_FP').
heapPtsTo('d_chrome_types', 'get', 'd_chrome_types_get').
heapPtsTo('d_chrome_types_get', 'prototype', 'p_chrome_types_get').
prototype('p_chrome_types_get', 'h_FP').
heapPtsTo('d_chrome_types', 'set', 'd_chrome_types_set').
heapPtsTo('d_chrome_types_set', 'prototype', 'p_chrome_types_set').
prototype('p_chrome_types_set', 'h_FP').

% chrome.webNavigation object
heapPtsTo('d_chrome', 'webNavigation', 'd_chrome_webNavigation').

% chrome.webNavigation.* method
heapPtsTo('d_chrome_webNavigation', 'getAllFrames', 'd_chrome_webNavigation_getAllFrames').
heapPtsTo('d_chrome_webNavigation_getAllFrames', 'prototype', 'p_chrome_webNavigation_getAllFrames').
prototype('p_chrome_webNavigation_getAllFrames', 'h_FP').
heapPtsTo('d_chrome_webNavigation', 'getFrame', 'd_chrome_webNavigation_getFrame').
heapPtsTo('d_chrome_webNavigation_getFrame', 'prototype', 'p_chrome_webNavigation_getFrame').
prototype('p_chrome_webNavigation_getFrame', 'h_FP').

% chrome.webRequest object
heapPtsTo('d_chrome', 'webRequest', 'd_chrome_webRequest').

% chrome.webRequest.* methods
heapPtsTo('d_chrome_webRequest', 'handlerBehaviorChanged', 'd_chrome_webRequest_handlerBehaviorChanged').
heapPtsTo('d_chrome_webRequest_handlerBehaviorChanged', 'prototype', 'p_chrome_webRequest_handlerBehaviorChanged').
prototype('p_chrome_webRequest_handlerBehaviorChanged', 'h_FP').

% chrome.windows object
heapPtsTo('d_chrome', 'windows', 'd_chrome_windows').

% chrome.windows.* methods
heapPtsTo('d_chrome_windows', 'create', 'd_chrome_windows_create').
heapPtsTo('d_chrome_windows_create', 'prototype', 'p_chrome_windows_create').
prototype('p_chrome_windows_create', 'h_FP').
heapPtsTo('d_chrome_windows', 'get', 'd_chrome_windows_get').
heapPtsTo('d_chrome_windows_get', 'prototype', 'p_chrome_windows_get').
prototype('p_chrome_windows_get', 'h_FP').
heapPtsTo('d_chrome_windows', 'getAll', 'd_chrome_windows_getAll').
heapPtsTo('d_chrome_windows_getAll', 'prototype', 'p_chrome_windows_getAll').
prototype('p_chrome_windows_getAll', 'h_FP').
heapPtsTo('d_chrome_windows', 'getCurrent', 'd_chrome_windows_getCurrent').
heapPtsTo('d_chrome_windows_getCurrent', 'prototype', 'p_chrome_windows_getCurrent').
prototype('p_chrome_windows_getCurrent', 'h_FP').
heapPtsTo('d_chrome_windows', 'getLastFocused', 'd_chrome_windows_getLastFocused').
heapPtsTo('d_chrome_windows_getLastFocused', 'prototype', 'p_chrome_windows_getLastFocused').
prototype('p_chrome_windows_getLastFocused', 'h_FP').
heapPtsTo('d_chrome_windows', 'remove', 'd_chrome_windows_remove').
heapPtsTo('d_chrome_windows_remove', 'prototype', 'p_chrome_windows_remove').
prototype('p_chrome_windows_remove', 'h_FP').
heapPtsTo('d_chrome_windows', 'update', 'd_chrome_windows_update').
heapPtsTo('d_chrome_windows_update', 'prototype', 'p_chrome_windows_update').
prototype('p_chrome_windows_update', 'h_FP').

%%% End of chrome.pl %%%

